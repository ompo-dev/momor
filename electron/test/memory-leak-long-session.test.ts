...
// ── Test helpers ───────────────────────────────────────────────────────────
function verifySourceContains(
  filePath: string,
  patterns: Array<{ pat: string | RegExp; label: string; invert?: boolean }>,
...
async function main() {
  ...
  const llmChecks = verifySourceContains(
    ...
    [
      {
        pat: "public scrubKeys()",
        label: "scrubKeys() method declared public",
      },
      ...
      {
        pat: "Buffer.alloc",
        label: "Buffer.alloc found in LLMHelper source",
        invert: true,
      },
    ],
  );
  ...
  const stChecks = verifySourceContains(
    ...
    [
      {
        pat: "maxContextItems",
        label: "has maxContextItems limit (prevents unbounded growth)",
      },
      {
        pat: "contextWindowDuration",
        label: "has contextWindowDuration (time-based eviction)",
      },
      {
        pat: "evictOldEntries",
        label: "has evictOldEntries method for array bounds cleanup",
      },
      {
        pat: "compactTranscriptIfNeeded",
        label: "has compactTranscriptIfNeeded() (transcript compaction)",
      },
      {
        pat: /fullTranscript\.push/,
        label: "fullTranscript uses push (not unbounded concat)",
      },
      {
        pat: /fullTranscript\s*=\s*\[\]/,
        label: "fullTranscript is reset to [] (not accumulated forever)",
      },
      {
        pat: "removeAllListeners",
        label:
          "removeAllListeners NOT called in SessionTracker (good — uses per-event named cleanup instead)",
        invert: true,
      },
      {
        pat: /while\s*\(\s*(?:true|1)\s*\)/,
        label: "no while(true/1) infinite loops in SessionTracker",
        invert: true,
      },
    ],
  );
  ...
  const stopChecks = verifySourceContains(
    ...
    [
      {
        pat: "this.removeAllListeners()",
        label: "SystemAudioCapture.destroy() calls removeAllListeners()",
      },
      {
        pat: "this.monitor = null",
        label: "SystemAudioCapture sets monitor to null after destroy",
      },
      {
        pat: "if (!this.isRecording) return",
        label:
          "SystemAudioCapture has post-stop guard (prevents events after stop)",
      },
    ],
  );
  console.log();
  ...
  for (const af of audioFiles) {
    ...
    console.log(
      `  ${af}: ${hasRedundantCopy ? "❌ redundant Buffer.copy pattern found in executable code" : "✅ no redundant Buffer.copy in executable code"}`,
    );
  ...
  console.log(
    "  Full E2E leak test requires Electron runtime (app.getPath needed).",
  );
  console.log(
    "  Run the Playwright E2E test for runtime memory behavior validation.",
  );
