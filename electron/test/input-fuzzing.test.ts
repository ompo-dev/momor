...
const FUZZ_CASES: Array<{
    ...
    inputs: [
      {
        ...
        value: { speaker: 123, text: "hello" },
        expectedSafe: true,
      },
      {
        label: "speaker=object",
        value: { speaker: {}, text: "hello" },
        expectedSafe: true,
      },
      ...
      {
        label: "both null",
        value: { speaker: null, text: null },
        expectedSafe: true,
      },
      {
        label: "wrong types",
        value: { speaker: [], text: 0 },
        expectedSafe: true,
      },
      { label: "empty object", value: {}, expectedSafe: true },
      {
        label: "extra dangerous field",
        value: { speaker: "I", text: "hi", dangerous: true },
        expectedSafe: true,
      },
      {
        label: "unicode speaker",
        value: { speaker: "🎤", text: "hello" },
        expectedSafe: true,
      },
      {
  ...
  },
  {
    ...
    inputs: [
      {
        label: "empty name",
        value: { name: "", templateType: "general" },
        expectedSafe: true,
      },
      {
        label: "20KB name",
        value: { name: "A".repeat(20_000), templateType: "general" },
        expectedSafe: true,
      },
      {
        label: "SQL injection name",
        value: { name: "'; DROP TABLE modes; --", templateType: "general" },
        expectedSafe: true,
      },
      {
        label: "XSS name",
      ...
      {
        label: "null name",
        value: { name: null, templateType: "general" },
        expectedSafe: true,
      },
      {
        label: "undefined name",
        value: { name: undefined, templateType: "general" },
        expectedSafe: true,
      },
      {
        label: "templateType=number",
        value: { name: "valid", templateType: 123 },
        expectedSafe: true,
      },
      {
        label: "templateType=SQL",
        value: {
          name: "valid",
          templateType: "'; UPDATE modes SET admin=1; --",
        },
        expectedSafe: true,
      },
      { label: "empty object", value: {}, expectedSafe: true },
      {
        label: "only templateType",
        value: { templateType: "general" },
        expectedSafe: true,
      },
    ],
  },
  {
    ...
    inputs: [
      {
        label: "200KB customContext",
        value: { customContext: "A".repeat(200_000) },
        expectedSafe: true,
      },
      {
        label: "null bytes in context",
        value: { customContext: "\x00null\x00byte" },
        expectedSafe: true,
      },
      {
        label: "control chars",
        value: { customContext: "\x1f\x1e\x1f" },
        expectedSafe: true,
      },
      {
        label: "empty customContext",
        value: { customContext: "" },
        expectedSafe: true,
      },
      {
        label: "SQL injection context",
        value: { customContext: "'; DROP TABLE modes; --" },
        expectedSafe: true,
      },
      {
        label: "XSS context",
        value: { customContext: "<script>alert('xss')</script>" },
        expectedSafe: true,
      },
      {
        label: "only name update",
        value: { name: "updated" },
        expectedSafe: true,
      },
      ...
      {
        label: "unicode context",
        value: { customContext: "你好こんにちは🎤" },
        expectedSafe: true,
      },
    ],
  },
...
async function main() {
  ...
  console.log("─".repeat(60));
  for (const input of FUZZ_CASES.find((c) =>
    c.handler.includes("generate-what-to-say"),
  )!.inputs) {
    const result = testQuestionInput(input.label, input.value);
    const sql =
      typeof input.value === "string" && wouldCauseSQLInjection(input.value);
    allResults.push({
      handler: "generate-what-to-say",
      label: input.label,
      ...result,
      sqlInjection: sql,
    });
    console.log(
      `  ${result.pass ? "✅" : "❌"} ${input.label}${result.error ? ` (${result.error})` : ""}${sql ? " [SQL patt]" : ""}`,
    );
  }
  ...
  console.log("─".repeat(60));
  for (const input of FUZZ_CASES.find((c) =>
    c.handler.includes("test-inject-transcript"),
  )!.inputs) {
    ...
    console.log(
      `  ${result.pass ? "✅" : "❌"} ${input.label}${result.error ? ` (${result.error})` : ""}`,
    );
  }
  ...
  console.log("─".repeat(60));
  for (const input of FUZZ_CASES.find((c) =>
    c.handler.includes("modes:create"),
  )!.inputs) {
    const result = testModesParamsInput(input.label, input.value);
    const sql =
      typeof input.value?.name === "string" &&
      wouldCauseSQLInjection(input.value.name);
    allResults.push({
      handler: "modes:create",
      label: input.label,
      ...result,
      sqlInjection: sql,
    });
  ...
  console.log("─".repeat(60));
  for (const input of FUZZ_CASES.find((c) =>
    c.handler.includes("modes:update"),
  )!.inputs) {
    const result = testModesUpdateInput(input.label, input.value);
    const sql =
      typeof input.value?.customContext === "string" &&
      wouldCauseSQLInjection(input.value.customContext);
    allResults.push({
      handler: "modes:update",
      label: input.label,
      ...result,
      sqlInjection: sql,
    });
  ...
  if (sqlDetections > 0)
    console.log(
      `  SQL injection patterns detected: ${sqlDetections} (DB uses parameterized queries — safe)`,
    );
  console.log("═".repeat(70));
  ...
  if (allPassed) {
    console.log(
      "✅ ALL INPUTS HANDLED SAFELY — no crashes, no unhandled exceptions",
    );
    console.log(
      "   Note: SQL injection patterns are detected but DB uses parameterized",
    );
    console.log(
      "   queries so these are already mitigated. XSS is handled by escaping",
    );
    console.log(
      "   in the LLM prompt assembly layer (WhatToAnswerLLM.escapeXml).",
    );
  } else {
