...
export class ElevenLabsStreamingSTT extends EventEmitter {
  ...
  constructor(apiKey: string) {
    ...
    if (process.env.NODE_ENV === "development") {
      try {
        ...
        console.log(
          `[ElevenLabsStreaming] Audio debug stream opened at: ${debugPath}`,
        );
      } catch (e) {
  ...
  /** Recognition language - maps momor key to ISO-639-1 for ElevenLabs, or 'auto' to omit code */
  public setRecognitionLanguage(key: string): void {
    ...
    if (this.languageCode !== newCode) {
      console.log(
        `[ElevenLabsStreaming] Language changed: ${this.languageCode || "(auto)"} -> ${newCode || "(auto)"}`,
      );
  ...
  public finalize(): void {
    ...
    if (this.pcmAccumulatorLen > 0) {
      ...
      try {
        ...
        console.log(
          "[ElevenLabsStreaming] Finalize — flushed pending accumulator",
        );
      } catch (err) {
   ...
   * Note: Input from momor DSP is 32-bit Float PCM (F32).
   */
  public write(chunk: Buffer): void {
    ...
    if (
      ...
      if (this.buffer.length > 500) {
        ...
        console.warn(
          "[ElevenLabsStreaming] Buffer full — oldest audio chunk dropped.",
        );
      }

      if (!this.isConnecting && this.shouldReconnect && !this.reconnectTimer) {
        console.log(
          "[ElevenLabsStreaming] WS not ready. Lazy connecting on new audio...",
        );
        this.connect();
      }
      return;
    ...
    try {
      ...
      if (this.debugWriteStream) {
        ...
        this.debugWriteStream.write(
          Buffer.from(
            outputS16.buffer,
            outputS16.byteOffset,
            outputS16.byteLength,
          ),
        );
      }
  ...
  private connect(): void {
    ...
    console.log(`[ElevenLabsStreaming] Connecting`, {
    ...
    console.log(
      `[ElevenLabsStreaming] Connecting with URL: ${url.replace(this.apiKey, "***")}`,
    );

    // streamingStttWsOptions: IPv4-only DNS + 15s handshake cap (dnsHelpers.ts).
    this.ws = new WebSocket(
      url,
      streamingStttWsOptions({
        headers: {
    ...
    this.ws.on("message", (data: WebSocket.RawData) => {
      try {
        ...
        if (this.debugMessageCount < 10) {
          console.log(
            `[ElevenLabsStreaming] RAW[${this.debugMessageCount}]:`,
            rawStr,
          );
        ...
        switch (msgType) {
          ...
          case "auth_error":
            console.error(
              "[ElevenLabsStreaming] Auth error — check key scope/permissions in ElevenLabs dashboard:",
              msg,
            );
            this.emit("error", msg);
          ...
          default:
            ...
            if (msg.error) {
              ...
              console.log(
                "[ElevenLabsStreaming] Received message:",
                msgType,
                Object.keys(msg),
              );
            }
    ...
    this.ws.on("close", (code, reason) => {
      ...
      this.isSessionReady = false;
      console.log(
        `[ElevenLabsStreaming] Closed: code=${code} reason=${reason}`,
      );
  ...
  private scheduleReconnect(): void {
    ...
    if (this.reconnectAttempts >= RECONNECT_MAX_ATTEMPTS) {
      console.error(
        `[ElevenLabsStreaming] Max reconnect attempts (${RECONNECT_MAX_ATTEMPTS}) reached — giving up`,
      );
      ...
      this.emit(
        "error",
        new Error("ElevenLabsStreamingSTT: max reconnect attempts exceeded"),
      );
      return;
    }
    ...
    console.log(
      `[ElevenLabsStreaming] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts}/${RECONNECT_MAX_ATTEMPTS})...`,
    );
