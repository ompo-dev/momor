 ...
 * momorProSTT
 *
 * Connects to the momor API WebSocket transcription endpoint.
 ...
 */
export class momorProSTT extends EventEmitter {
  ...
  private readonly BACKEND_URL = ""; // disabled in open-source build
  ...
  public setSampleRate(rate: number): void {
    ...
    console.log(
      `[momorProSTT:${this.channel}] Sample rate ${previousRate}Hz → ${rate}Hz`,
    );
    ...
    if (this.isActive && this.isConnected) {
      console.log(
        `[momorProSTT:${this.channel}] Rate changed mid-stream — reconnecting WS so server uses the new declared rate.`,
      );
  ...
  public setRecognitionLanguage(key: string): void {
    ...
    if (key === "auto") {
      ...
      console.log("[momorProSTT] Language set to auto-detect mode");
    } else {
      const config = RECOGNITION_LANGUAGES[key];
      if (!config) {
        console.warn(`[momorProSTT] Unknown language key: ${key}`);
        return;
      }
      this.languageBcp47 = config.bcp47;
      this.languageAlternates =
        "alternates" in config ? (config as EnglishVariant).alternates : [];
      console.log(
        `[momorProSTT] Language set: ${key} → ${this.languageBcp47}`,
        this.languageAlternates.length
          ? `(alts: ${this.languageAlternates.join(", ")})`
          : "",
      );
    }
    ...
    if (this.isActive && this.isConnected) {
      console.log("[momorProSTT] Language changed while active — reconnecting");
  ...
  /** No-op — momor API server handles VAD internally */
  public notifySpeechEnded(): void {}

  /** No-op — momor API server finalizes via VAD; no client-side flush available */
  ...
  public write(chunk: Buffer): void {
    ...
    if (
      ...
      if (this.buffer.length > this.BUFFER_MAX_CHUNKS) {
        ...
        if (!this.bufferOverflowReported) {
          ...
          console.warn(
            `[momorProSTT:${this.channel}] Buffer overflow — dropping oldest chunks. Reconnect taking too long; transcript will have a gap.`,
          );
      ...
      if (this.buffer.length <= 3 || this.buffer.length % 100 === 0) {
        const wsState = this.ws
          ...
          : "null";
        console.log(
          `[momorProSTT:${this.channel}] Buffering chunk (buffer=${this.buffer.length}, isConnected=${this.isConnected}, ws=${wsState})`,
        );
      }
      return;
    }
    ...
    if (this._chunksSent <= 5 || this._chunksSent % 200 === 0) {
      console.log(
        `[momorProSTT:${this.channel}] Sent chunk #${this._chunksSent} (${chunk.length}B) to server`,
      );
    }
    this.ws.send(chunk);
  }
  ...
  private connect(_skipStagger = false): void {
    throw new Error(
      "momor Pro STT has been removed. Please use a local or cloud STT provider instead.",
    );
  }

  private _connect_disabled(skipStagger = false): void {
    ...
    if (!skipStagger) {
      ...
      const now = Date.now();
      const reserved = momorProSTT.nextSlotByKey.get(this.apiKey) ?? 0;
      const staggerMs = Math.max(0, reserved - now);
      momorProSTT.nextSlotByKey.set(
        this.apiKey,
        Math.max(now, reserved) + momorProSTT.SLOT_INTERVAL_MS,
      );

      if (staggerMs > 0) {
        this.isConnecting = true; // Hold the slot while waiting
        console.log(
          `[momorProSTT:${this.channel}] Staggering connection ${staggerMs}ms (concurrent key collision prevention)`,
        );
        setTimeout(() => {
    ...
    console.log(
      `[momorProSTT] Connecting (attempt ${this.reconnectAttempts + 1})...`,
    );
      ...
      guard(() => {
        ...
        if (this.apiKey === TRIAL_SENTINEL_KEY) {
          try {
            ...
            /* CredentialsManager unavailable — connection will be rejected by server */
          }
        } else {
      ...
      guard(() => {
        try {
          ...
          if (!msg.text || msg.is_final) {
            console.log(`[momorProSTT:${this.channel}] Server msg`, {
              type: msg.type,
          ...
          if (msg.status === "connected") {
            ...
            console.log(`[momorProSTT] Connected via ${msg.provider}`);
          ...
          if (msg.language_detected) {
            ...
            console.log(`[momorProSTT] Auto-detected language: ${detected}`);
          ...
          console.error("[momorProSTT] Parse error:", err);
        }
      }),
    );
    ...
    ws.on("error", (err: Error & { code?: string }) =>
      guard(() => {
        ...
        if (this.isDnsFailure) {
          console.warn(
            `[momorProSTT:${this.channel}] DNS failure (${err.code}) — will retry in ${this.DNS_RETRY_MS / 1000}s without burning backoff`,
          );
        } else {
          console.error("[momorProSTT] WebSocket error:", err.message);
        }
        this.isConnecting = false;
        this.isConnected = false;
        this.emit("error", err);
    ...
    ws.on("close", (code: number) =>
      guard(() => {
        this.isConnecting = false;
        this.isConnected = false;
        console.log(`[momorProSTT] Connection closed (code ${code})`);
  ...
  private scheduleReconnect(): void {
    ...
    if (this.isDnsFailure) {
      ...
      console.warn(
        `[momorProSTT] DNS retry in ${this.DNS_RETRY_MS / 1000}s...`,
      );
    ...
    const exp =
      this.RECONNECT_BASE_MS * Math.pow(2, Math.min(this.reconnectAttempts, 6));
    ...
    console.log(
      `[momorProSTT:${this.channel}] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts})...`,
    );
  ...
  private flushBuffer(): void {
    ...
    if (this.bufferDroppedChunks > 0) {
      console.warn(
        `[momorProSTT:${this.channel}] Reconnected — flushing ${pending.length} buffered chunks; ${this.bufferDroppedChunks} were dropped during outage`,
      );
    }
