...
import {
  ...
  type UserSessionSyncPayload,
} from "./services/UserSessionContextService";
import { mergeLiveMeetingTranscriptIntoContext } from "./services/LiveMeetingContext";
...
export function initializeIpcHandlers(appState: AppState): void {
  ...
  /**
   * Always returns true — all features are unlocked in this open-source build.
   */
  ...
  safeHandle(
    ...
    async (event, { width, height }: { width: number; height: number }) => {
      ...
      if (
        ...
        // momorInterface logic - Resize ONLY the overlay window using dedicated method
  ...
  // Generate suggestion from transcript - momor-style text-only reasoning
  safeHandle(
    "generate-suggestion",
  ...
  safeHandle("start-answer-now-mic", async () => {
    appState.startAnswerNowMic();
  });

  safeHandle("stop-answer-now-mic", async () => {
    appState.stopAnswerNowMic();
  });
  ...
  // Prevents the small fast-mode model from over-firing the "I'm momor" canned reply
  // (which used to escape the prompt's hard rule for any ambiguous input).
  const IDENTITY_PROBE_RE =
    /^\s*(who\s+(are|r)\s+(you|u|this|momor)|what\s+(are|r)\s+(you|u)|are\s+you\s+(chatgpt|gpt[-\s]?\d?|claude|gemini|llama|an?\s+(ai|bot|llm|model|assistant))|what('?s|\s+is)\s+your\s+(name|model)|which\s+(ai|model|llm)\s+are\s+you|who\s+(made|built|created|developed|trained)\s+(you|this|momor)|what\s+model\s+(are\s+you|do\s+you\s+use)|introduce\s+yourself)\s*\??\s*$/i;
  const CREATOR_PROBE_RE =
    /^\s*(who\s+(made|built|created|developed|trained)\s+(you|this|momor))\s*\??\s*$/i;
  ...
  safeHandle("get-log-file-path", async () => {
    try {
      return path.join(app.getPath("documents"), "momor_debug.log");
    } catch {
      return null;
    }
  });
  ...
  safeHandle("open-log-file", async () => {
    try {
      const logPath = path.join(app.getPath("documents"), "momor_debug.log");
  ...
  safeHandle("setDeepseekApiKey", async (_, key: string) => {
    try {
      const { CredentialsManager } = require("./services/CredentialsManager");
      const cm = CredentialsManager.getInstance();
      cm.setDeepseekApiKey(key);
      if (key) {
        const llmHelper = appState.processingHelper.getLLMHelper();
        llmHelper.initDeepseek(key, cm.getDeepseekPreferredModel());
      }
      return { success: true };
    } catch (error: any) {
  ...
  safeHandle("setDeepseekModel", async (_, model: string) => {
    try {
      const { CredentialsManager } = require("./services/CredentialsManager");
      const cm = CredentialsManager.getInstance();
      cm.setDeepseekPreferredModel(model);
      const deepseekKey = cm.getDeepseekApiKey();
      if (deepseekKey) {
        const llmHelper = appState.processingHelper.getLLMHelper();
        llmHelper.initDeepseek(deepseekKey, model);
      }
      return { success: true };
    } catch (error: any) {
  ...
  safeHandle("getDeepseekApiKey", async () => {
    try {
      const { CredentialsManager } = require("./services/CredentialsManager");
      return CredentialsManager.getInstance().getDeepseekApiKey() ?? "";
    } catch {
      return "";
    }
  });
  ...
  safeHandle(
    ...
    async (
      ...
      try {
        const { CredentialsManager } = require("./services/CredentialsManager");
        const cm = CredentialsManager.getInstance();
        cm.setLlmBackupApiKeys(provider, Array.isArray(keys) ? keys : []);
        return { success: true };
      } catch (error: any) {
        return { success: false, error: error.message };
      }
  ...
  safeHandle(
    "setWhisperXConfig",
    async (_, config: { url: string; enabled: boolean }) => {
      const { CredentialsManager } = require("./services/CredentialsManager");
      const cm = CredentialsManager.getInstance();
      cm.setWhisperXUrl(config.url);
      cm.setWhisperXEnabled(config.enabled);
      return { success: true };
    },
  );

  safeHandle("getWhisperXConfig", async () => {
    const { CredentialsManager } = require("./services/CredentialsManager");
    const cm = CredentialsManager.getInstance();
    return {
      url: cm.getWhisperXUrl() ?? "http://localhost:8000",
      enabled: cm.isWhisperXEnabled(),
    };
  });
  ...
  safeHandle("testOpenClaudeConnection", async () => {
    try {
      ...
      const result = await OpenClaudeService.run(config, {
    ...
    async (
      ...
      try {
        ...
        if (payload.kind === "codex") {
          return OpenClaudeService.spawnInteractiveLogin("codex", cliPath);
        }
        const loginResult = await OpenClaudeService.authLogin({
          enabled: true,
        ...
        return OpenClaudeService.spawnInteractiveLogin("openclaude", cliPath);
      } catch (e: any) {
        return { success: false, error: e.message || "Failed to start login" };
      }
    },
  );
  ...
  safeHandle("get-openclaude-models", async () => {
    try {
      const { OpenClaudeService } =
        await import("./services/OpenClaudeService");
      const config = OpenClaudeService.readGlobalConfig();
      const profiles = OpenClaudeService.listProfilesFromConfig(config);
      const models = OpenClaudeService.listAvailableModels(config);
      const activeProfileId =
        typeof config?.activeProviderProfileId === "string"
          ? config.activeProviderProfileId
          : null;
      return { success: true, models, profiles, activeProfileId };
    } catch (e: any) {
      return { success: false, error: e.message, models: [], profiles: [] };
    }
  });
  ...
  safeHandle("set-momor-api-key", async (_, apiKey: string) => {
    try {
      const { CredentialsManager } = require("./services/CredentialsManager");
      const cm = CredentialsManager.getInstance();
      const prevSttProvider = cm.getSttProvider();
      cm.setmomorApiKey(apiKey);

      // Update LLMHelper immediately (same pattern as other provider keys)
      const llmHelper = appState.processingHelper.getLLMHelper();
      llmHelper.setmomorKey(apiKey || null);
      ...
      // If setmomorApiKey auto-promoted the STT provider to 'momor', reconfigure
      ...
      if (newSttProvider !== prevSttProvider) {
        console.log(
          `[IPC] set-momor-api-key: STT provider changed ${prevSttProvider} → ${newSttProvider}, reconfiguring pipeline`,
        );
        await appState.reconfigureSttProvider();
      }

      // Auto-activate momor Pro for pro/max/ultra API plans.
      ...
      if (apiKey) {
        try {
          ...
          if (result.success) {
            console.log(
              "[IPC] set-momor-api-key: Pro auto-activated via API plan.",
            );
            ...
            console.log(
              "[IPC] set-momor-api-key: existing Gumroad/Dodo license preserved — Pro not overwritten.",
            );
          } else {
            console.log(
              "[IPC] set-momor-api-key: Pro not activated —",
              result.error,
            );
          }
        } catch (e: any) {
          ...
          console.warn(
            "[IPC] set-momor-api-key: LicenseManager unavailable for Pro auto-activation:",
            e?.message,
          );
        }
      } else {
        // API key was cleared — deactivate any momor_api Pro license so premium is revoked.
        try {
          const {
            LicenseManager,
          } = require("../premium/electron/services/LicenseManager");
          const lm = LicenseManager.getInstance();
          // Only deactivate if the stored license is from a momor_api subscription.
          // Never touch Gumroad/Dodo lifetime licenses here.
          const details = lm.getLicenseDetails();
          if (details.isPremium && details.provider === "momor_api") {
            await lm.deactivate();
            console.log(
              "[IPC] set-momor-api-key: key cleared — momor_api Pro license deactivated.",
            );
          ...
          console.warn(
            "[IPC] set-momor-api-key: LicenseManager unavailable for Pro deactivation on key clear:",
            e?.message,
          );
        }
      }
      ...
      console.error("Error saving momor API key:", error);
      return { success: false, error: error.message };
    } finally {
  ...
  safeHandle("get-momor-usage", async () => {
    try {
      const { CredentialsManager } = require("./services/CredentialsManager");
      const key = CredentialsManager.getInstance().getmomorApiKey();
      ...
      // api.momor.software is disabled in the open-source build.
      return { ok: false, error: "momor_api_disabled" };
      const res = await fetch("https://api.momor.software/v1/usage", {
        headers: { "x-momor-key": key },
  ...
  safeHandle("invalidate-momor-usage-cache", () => {
    _usageCache.clear();
    return { ok: true };
  });
  ...
  safeHandle("get-stt-provider", async () => {
    try {
      ...
      return CredentialsManager.getInstance().getSttProvider(
        appState.getSessionSttProfileId(),
      );
    } catch (error: any) {
      return "none";
    }
  ...
  safeHandle("get-stt-profiles", async () => {
    try {
      const { CredentialsManager } = require("./services/CredentialsManager");
      const { isSttProfileConfigured } = require("./services/sttProfiles");
      const cm = CredentialsManager.getInstance();
      const creds = cm.getAllCredentials();
      const profiles = cm.getMaskedSttProfiles();
      const sessionId = appState.getSessionSttProfileId();
      const defaultId = cm.getDefaultSttProfileId();
      const active = cm.resolveActiveSttProfile(sessionId);
      return {
        success: true,
        profiles,
        defaultProfileId: defaultId,
        sessionProfileId: sessionId,
        activeProfileId: active?.id ?? null,
        activeKind: active?.kind ?? "none",
        activeConfigured: active
          ? isSttProfileConfigured(active, creds)
          : false,
      };
    } catch (error: any) {
  ...
  safeHandle("delete-stt-profile", async (_, profileId: string) => {
    try {
      const { CredentialsManager } = require("./services/CredentialsManager");
      const cm = CredentialsManager.getInstance();
      cm.deleteSttProfile(profileId);
      if (appState.getSessionSttProfileId() === profileId) {
        appState.setSessionSttProfileId(null);
      }
      await appState.reconfigureSttProvider();
      BrowserWindow.getAllWindows().forEach((win) => {
        if (!win.isDestroyed()) win.webContents.send("credentials-changed");
  ...
  safeHandle("set-default-stt-profile", async (_, profileId: string) => {
    try {
      const { CredentialsManager } = require("./services/CredentialsManager");
      const cm = CredentialsManager.getInstance();
      cm.setDefaultSttProfileId(profileId || null);
      appState.setSessionSttProfileId(null);
      await appState.reconfigureSttProvider();
      BrowserWindow.getAllWindows().forEach((win) => {
        if (!win.isDestroyed()) {
          win.webContents.send("credentials-changed");
          win.webContents.send("stt-profile-changed", { profileId });
        }
      });
      return { success: true };
    } catch (error: any) {
  ...
  safeHandle("set-stt-profile", async (_, profileId: string) => {
    try {
      const { CredentialsManager } = require("./services/CredentialsManager");
      const cm = CredentialsManager.getInstance();
      const profile = cm
      ...
      appState.setSessionSttProfileId(profileId);
      cm.applyActiveSttProfileToLegacy(profileId);
      await appState.reconfigureSttProvider();
      BrowserWindow.getAllWindows().forEach((win) => {
        if (!win.isDestroyed()) {
          win.webContents.send("stt-profile-changed", {
            profileId,
  ...
  safeHandle("add-stt-preset", async (_, kind: string, name?: string) => {
    try {
      const { CredentialsManager } = require("./services/CredentialsManager");
      const cm = CredentialsManager.getInstance();
      const before = cm.getSttProfiles();
      const existed = before.some((p: { kind: string }) => p.kind === kind);
      const profile = cm.addSttPreset(kind as any, name);
      BrowserWindow.getAllWindows().forEach((win) => {
        if (!win.isDestroyed()) win.webContents.send("credentials-changed");
      });
      return {
  ...
  safeHandle("local-whisper-set-model", async (_, modelId: string) => {
    try {
      ...
      const { CredentialsManager } = require("./services/CredentialsManager");
      CredentialsManager.getInstance().syncLocalWhisperModelOnProfiles(modelId);
      await appState.reconfigureSttProvider();
      return { success: true };
    } catch (e: any) {
      return { success: false, error: e.message };
    ...
    try {
      ...
      w.on("message", (msg: any) => {
        ...
        if (msg.type === "progress") {
          sender.send("local-whisper-download-progress", {
            modelId,
          ...
          activeWhisperDownloads.delete(modelId);
          sender.send("local-whisper-download-error", {
            modelId,
            error: msg.message,
          });
          w.terminate();
  ...
  safeHandle("test-codex-cli", async (_, config?: any) => {
    try {
      ...
      if (
        result.success &&
        result.resolvedPath &&
        result.resolvedPath !== normalized.path
      ) {
  ...
  safeHandle("set-model", async (_, modelId: string) => {
    try {
      ...
      // Broadcast to all windows so momorInterface can update its selector (session-only update)
      BrowserWindow.getAllWindows().forEach((win) => {
        if (!win.isDestroyed()) {
    ...
    try {
      ...
      if (modelId.startsWith("deepseek")) {
        ...
        if (deepseekKey) {
          llmHelper.initDeepseek(
            deepseekKey,
            cm.getDeepseekPreferredModel() || modelId,
          );
        }
      }
      ...
      // Broadcast to all windows so momorInterface can update its selector
      BrowserWindow.getAllWindows().forEach((win) => {
        if (!win.isDestroyed()) {
          win.webContents.send("model-changed", modelId);
        }
  ...
  safeHandle("set-vision-default-model", async (_, modelId: string) => {
    try {
      const { CredentialsManager } = require("./services/CredentialsManager");
      const cm = CredentialsManager.getInstance();
      cm.setVisionDefaultModel(modelId);
      BrowserWindow.getAllWindows().forEach((win) => {
        if (!win.isDestroyed()) {
          win.webContents.send("vision-model-changed", modelId);
        }
      });
      return { success: true };
    } catch (error: any) {
      console.error("Error setting vision default model:", error);
      return { success: false, error: error.message };
    }
  });

  safeHandle("get-vision-default-model", async () => {
    try {
      const { CredentialsManager } = require("./services/CredentialsManager");
      const cm = CredentialsManager.getInstance();
      return { model: cm.getVisionDefaultModel() };
    } catch (error: any) {
      console.error("Error getting vision default model:", error);
      return { model: "gemini-3.1-flash-lite-preview" };
    }
  });
  ...
  // momor answers from the image directly via a vision-capable provider. Do not re-introduce
  ...
  safeHandle("set-active-listening-mode", async (_, enabled: unknown) => {
    try {
      const intelligenceManager = appState.getIntelligenceManager();
      intelligenceManager.setActiveListeningMode(enabled === true);
      return { success: true };
    } catch (error: any) {
      return { success: false, error: error.message };
    }
  ...
  safeHandle(
    "user-context:sync",
    async (_, payload: UserSessionSyncPayload) => {
      try {
        UserSessionContextService.getInstance().sync(payload);
        appState.processingHelper
          .getLLMHelper()
          .setCustomNotes(payload.personalContext ?? "");
        return { success: true };
      } catch (error: any) {
        console.error("[IPC] user-context:sync error:", error);
        return { success: false, error: error.message };
      }
    },
  );
  ...
  safeHandle("profile:research-company", async (_, companyName: string) => {
    try {
      ...
      // Wire search provider: Tavily (user key) → momor API (fallback) → none (LLM-only)
      const { CredentialsManager } = require("./services/CredentialsManager");
      const cm = CredentialsManager.getInstance();
      ...
      if (tavilyApiKey) {
        ...
        const momorKey = cm.getmomorApiKey();
        if (momorKey) {
          const {
            momorSearchProvider,
          } = require("../premium/electron/knowledge/momorSearchProvider");
          ...
          const trialToken =
            momorKey === TRIAL_SENTINEL_KEY ? cm.getTrialToken() : undefined;
          engine.setSearchProvider(
            new momorSearchProvider(momorKey, trialToken ?? undefined),
          );
          console.log(
            "[IPC] Company research: using momor API search (no Tavily key configured)",
          );
        }
      }
    ...
    try {
      ...
      try {
        ...
        return {
          success: false,
          error:
            "Could not read the selected file. It may have moved or been deleted.",
        };
      }
      if (!stats.isFile()) {
        return {
      ...
      return {
        success: false,
        error:
          "Could not read the selected file. Please try a different file or contact support.",
      };
    }
  });
  ...
  safeHandle("phone-mirror:set-lan", async (_, exposeOnLan: boolean) => {
    try {
      return await PhoneMirrorService.getInstance().setExposeOnLan(
        !!exposeOnLan,
      );
    } catch (e: any) {
