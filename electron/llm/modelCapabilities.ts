...
const TIER_BUDGETS: Record<
  ModelTier,
  { max: number; system: number; output: number }
> = {
...
function isCloudIdentifier(id: string): boolean {
  ...
  if (s === "momor" || s.startsWith("momor-")) return true;
...
function isLargeGroqModel(id: string): boolean {
  const s = id.toLowerCase();
  if (
    s.includes("llama-3.3-70b") ||
    s.includes("llama-3.1-70b") ||
    s.includes("llama3-70b")
  )
    return true;
...
function ollamaSupportsImages(id: string): boolean {
  const s = id.toLowerCase();
  return /llava|bakllava|moondream|llama3\.2-vision|llama-3\.2-vision|gemma3|minicpm-v|qwen2\.5-vl|qwen2-vl|pixtral/.test(
    s,
  );
}
...
export function getModelCapabilities(
  ...
  if (isOllama) {
    ...
    const tier: ModelTier =
      size != null && size >= 13 ? "local-large" : "local-small";
...
export function selectPromptTier(
  modelId: string,
  isOllama: boolean,
): PromptTier {
  return getModelCapabilities(modelId, isOllama).tier === "local-small"
    ? "tiny"
    : "full";
}
...
export function truncateTranscriptToFit(
  ...
  const total = (turns: TranscriptTurn[]) =>
    turns.reduce((s, t) => s + estimateTokens(t.text) + 6, 0);
