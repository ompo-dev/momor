...
test("routeLLMProviders returns deterministic text fallback order with availability", async () => {
  ...
  assert.equal(
    attempts.every((attempt) => attempt.status === "available"),
    true,
  );
  assert.deepEqual(
    attempts.map((attempt) => attempt.provider),
...
test("routeLLMProviders marks missing providers unavailable with reasons", async () => {
  ...
  assert.equal(
    attempts.every((attempt) => attempt.status === "unavailable"),
    true,
  );
  assert.equal(
    attempts.find((attempt) => attempt.provider === "codex").unavailableReason,
    "missing_config",
  );
  assert.equal(
    attempts.find((attempt) => attempt.provider === "openai").unavailableReason,
    "missing_api_key",
  );
});
...
test("routeLLMProviders marks unsupported capabilities without dropping attempts", async () => {
  ...
  assert.equal(
    attempts.find((attempt) => attempt.provider === "momor").unavailableReason,
    "unsupported_capability",
  );
  assert.equal(
    attempts.find((attempt) => attempt.provider === "gemini_flash")
      .unavailableReason,
    "unsupported_capability",
  );
  assert.equal(
    attempts.find((attempt) => attempt.provider === "gemini_pro").status,
    "available",
  );
  assert.equal(
    attempts.find((attempt) => attempt.provider === "openai").status,
    "available",
  );
});

test("routeLLMProviders does not mutate input objects", async () => {
  const availability = { hasmomor: true, hasGroq: false };
