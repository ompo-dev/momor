...
export type ProviderDataScope =
  ...
  | "embeddings"
  | "post_call_summary";
export type ProviderDataScopePolicy = Partial<
  Record<ProviderDataScope, boolean>
>;
...
export class ProviderScopeError extends Error {
  constructor(
    ...
    super(
      `Provider ${provider} blocked by data scope policy: ${deniedScopes.join(", ")}`,
    );
    this.name = "ProviderScopeError";
  }
}

export function getDeniedDataScopes(
  scopes: ProviderDataScope[] = [],
  policy?: ProviderDataScopePolicy,
): ProviderDataScope[] {
  return scopes.filter((scope) => policy?.[scope] === false);
}

export function assertProviderDataScopes(
  provider: string,
  scopes: ProviderDataScope[] = [],
  policy?: ProviderDataScopePolicy,
): void {
...
function statusFor(
  spec: ProviderSpec,
  capability: ProviderCapability,
  deniedScopes: ProviderDataScope[] = [],
): Pick<ProviderAttempt, "status" | "unavailableReason"> {
  if (!spec.supports.includes(capability)) {
    return {
      status: "unavailable",
      unavailableReason: "unsupported_capability",
    };
  }
  ...
  return {
    status: "unavailable",
    unavailableReason: spec.unavailableReason ?? "missing_api_key",
  };
}

export function routeLLMProviders(
  ...
  const momor: ProviderSpec = {
    provider: "momor",
    name: "momor API",
    model: models.momor,
    available: Boolean(availability.hasmomor),
  ...
  const groq: ProviderSpec = {
    ...
    unavailableReason: availability.groqDisabled
      ? "disabled"
      : "missing_api_key",
  ...
  const openclaude: ProviderSpec = {
    provider: "openclaude",
    name: `OpenClaude (${models.openclaude ?? "claude-sonnet-4-6"})`,
    model: models.openclaude,
    available: Boolean(availability.hasOpenClaude),
    unavailableReason: "missing_config",
    supports: ["chat", "stream_chat", "structured"],
  };

  const orderedSpecs = options.multimodal
    ? [momor, codex, openai, geminiFlash, claude, geminiPro, groq]
    : [momor, groq, codex, geminiFlash, geminiPro, openai, claude, openclaude];

  const deniedScopes = getDeniedDataScopes(
    options.dataScopes,
    options.scopePolicy,
  );
...
export class ProviderRouter {
  ...
  constructor(circuitConfig?: Partial<CircuitBreakerConfig>) {
    ...
    ["gemini", "groq", "openai", "claude", "momor", "codex"].forEach(
      (provider) => {
        this.circuitBreakers.set(
          provider,
          new CircuitBreaker(provider, config),
        );
      },
    );
  }
  ...
  selectProvider(policy: RoutingPolicy): ProviderChoice {
    ...
    const availableProviders = this.filterHealthyProviders(
      ["gemini", "groq", "openai", "claude", "momor", "codex"],
      health,
    );
    ...
    if (policy.needsVision) {
      const visionProvider = this.selectFromCapabilities(
        availableProviders,
        VISION_PROVIDERS,
        "vision",
        health,
    ...
    if (policy.preferLowLatency) {
      const fastProvider = this.selectFromCapabilities(
        availableProviders,
        LOW_LATENCY_PROVIDERS,
        "low-latency",
        health,
      );
    ...
    if (policy.actionType === "summary" || policy.actionType === "recap") {
      const qualityProvider = this.selectFromCapabilities(
        availableProviders,
        QUALITY_PROVIDERS,
        "quality",
        health,
    ...
    if (policy.mode) {
      const modeProvider = this.getModeProvider(
        policy.mode,
        availableProviders,
        health,
      );
  ...
  private getModeProvider(
    ...
    return this.selectFromCapabilities(
      available,
      preferences,
      `mode:${mode}`,
      health,
