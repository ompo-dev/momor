...
// exchange and refresh are proxied through momor-api, which holds the secret.
...
// Base URL for the momor-api proxy — disabled in open-source build.
// Override with momor_API_URL env var if you operate your own backend.
const momor_API_URL = (process.env.momor_API_URL || "").replace(/\/+$/, "");

if (GOOGLE_CLIENT_ID === "YOUR_CLIENT_ID_HERE") {
  console.warn(
    "[CalendarManager] GOOGLE_CLIENT_ID is using the default placeholder. Calendar features will not work until a valid client ID is provided via env var or build config.",
  );
}
...
export class CalendarManager extends EventEmitter {
  ...
  public async startAuthFlow(): Promise<void> {
    ...
    if (GOOGLE_CLIENT_ID === "YOUR_CLIENT_ID_HERE") {
      throw new Error(
        "GOOGLE_CLIENT_ID is not configured. Set it in .env and restart the app.",
      );
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      const server = http.createServer(async (req, res) => {
        try {
          if (req.url?.startsWith("/auth/callback")) {
            const qs = new url.URL(req.url, "http://localhost:11111")
              .searchParams;
            ...
            if (code) {
              res.end(
                "Authentication successful! You can close this window and return to Momor.",
              );
      ...
      const timeout = setTimeout(
        () => {
          finish(() =>
            reject(new Error("Calendar auth timed out — port released.")),
          );
        },
        5 * 60 * 1000,
      );
  ...
  private async exchangeCodeForToken(code: string) {
    try {
      // Proxied through momor-api so GOOGLE_CLIENT_SECRET never ships in the desktop app.
      // Fetch (vs. axios) so this call shares the global keep-alive pool with every other
      // request to api.momor.software and exposes the same error shape (res.ok / res.status)
      // as the rest of the codebase.
      const response = await fetch(`${momor_API_URL}/api/calendar/exchange`, {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `exchange_failed status=${response.status} ${(errBody as any).error || ""}`.trim(),
        );
      }
  ...
  private async refreshAccessToken() {
    ...
    try {
      // Proxied through momor-api so GOOGLE_CLIENT_SECRET never ships in the desktop app.
      const response = await fetch(`${momor_API_URL}/api/calendar/refresh`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  private showNotification(event: CalendarEvent) {
    ...
    const notif = new Notification({
      ...
      body: `"${event.title}" starts in 2 minutes. Start Momor?`,
      actions: [
  ...
  private async fetchEventsInternal(): Promise<CalendarEvent[]> {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          `[CalendarManager] Google Calendar fetch failed: HTTP ${response.status}`,
        );
        return [];
      }
      ...
      console.log(
        `[CalendarManager] Google returned ${items.length} raw items in next 7 days`,
      );
      ...
      console.log(
        `[CalendarManager] After filtering (timed, >=5min): ${filtered.length} events`,
      );
