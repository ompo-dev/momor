...
export class ModeHybridRetriever {
  ...
  private ensureIndexTable(): void {
    try {
      ...
      console.warn(
        "[ModeHybridRetriever] Failed to create index state table:",
        e,
      );
    }
  }
  ...
  private removeIndexState(fileId: string): void {
    try {
      this.db
        .prepare("DELETE FROM mode_reference_index_state WHERE file_id = ?")
        .run(fileId);
    } catch (e) {
  ...
  private computeFtsScore(chunk: string, queryWords: Set<string>): number {
    ...
    return (
      matches /
      Math.sqrt(queryWords.size * Math.max(1, new Set(chunkWords).size))
    );
  }
    ...
    try {
      ...
      telemetryService.track({
        ...
        properties: {
          ...
          // Optional test-run marker. Tests set momor_TELEMETRY_TEST_RUN_ID
          // to filter events emitted by their specific run, isolating
          // from any parallel test or stale JSONL line. Production
          // leaves this unset.
          testRunId: process.env.momor_TELEMETRY_TEST_RUN_ID || undefined,
        },
      });
    } catch {
  ...
  async retrieve(params: {
    ...
    if (this.isEmbeddingAvailable()) {
      try {
        candidates = await this.performHybridRetrieval(
          allCandidates,
          queryWords,
          queryText,
          adaptiveThreshold,
        ...
        console.warn(
          "[ModeHybridRetriever] Hybrid retrieval failed, falling back to lexical:",
          error,
        );
        ...
        candidates = this.performLexicalRetrieval(
          allCandidates,
          queryWords,
          adaptiveThreshold,
        );
      ...
      console.warn(
        "[ModeHybridRetriever] Embedding provider unavailable, using lexical fallback",
      );
      this.emitFallbackTelemetry({
  ...
  private async performHybridRetrieval(
    ...
    try {
      queryEmbedding =
        await this.embeddingPipeline.getEmbeddingForQuery(queryText);
    } catch (error) {
    ...
    try {
      if (typeof (this.embeddingPipeline as any).getEmbeddings === "function") {
        chunkEmbeddings = await (this.embeddingPipeline as any).getEmbeddings(
          chunkTexts,
        );
      } else {
      ...
      if (
        !Array.isArray(chunkEmbeddings) ||
        chunkEmbeddings.length !== chunkTexts.length
      ) {
        console.warn(
          `[ModeHybridRetriever] Batch embed returned ${chunkEmbeddings?.length ?? "undefined"} vectors for ${chunkTexts.length} chunks; vector path will be partially lexical-only.`,
        );
      ...
      console.warn(
        `[ModeHybridRetriever] Batch embed failed (${error instanceof Error ? error.message : String(error)}); degrading to lexical-only for this query.`,
      );
      chunkEmbeddings = [];
    }
    ...
    return scored.filter((c) => {
      const combined = this.combinedScore(
        c.ftsScore,
        c.vectorScore,
        FTS_WEIGHT,
      );
  ...
  private deduplicateChunks(candidates: ChunkCandidate[]): ChunkCandidate[] {
    ...
    for (const candidate of candidates) {
      ...
      const currentScore = this.combinedScore(
        candidate.ftsScore,
      ...
      if (!existing) {
        ...
        const existingScore = this.combinedScore(
          existing.ftsScore,
  ...
  private formatContext(chunks: ChunkCandidate[]): string {
    ...
    for (const chunk of chunks) {
      const combinedScore = this.combinedScore(
        chunk.ftsScore,
        chunk.vectorScore,
        FTS_WEIGHT,
      );
