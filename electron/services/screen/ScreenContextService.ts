...
// mode could be reintroduced. momor's default screen-understanding
...
import {
  getOcrProviderManager,
  OcrProviderManager,
} from "./OcrProviderManager";
...
export class ScreenContextService {
   ...
   */
  async captureCropper(
    captureArea?: Electron.Rectangle,
  ): Promise<ScreenContext> {
    const screenshotPath =
      await this.getScreenshotHelper().takeSelectiveScreenshot(captureArea);
    return this.captureScreenFromPath(screenshotPath);
  }
  ...
  async captureScreenFromPath(screenshotPath: string): Promise<ScreenContext> {
    ...
    try {
      ...
      console.warn(
        "[ScreenContextService] Failed to compute perceptual hash, using quick hash:",
        error,
      );
    ...
    try {
      ...
      const result = await ocrManager.recognize(screenshotPath, {
        timeoutMs: 8_000,
        maxDimension: 1200,
      });
      ocrText = result.text;
