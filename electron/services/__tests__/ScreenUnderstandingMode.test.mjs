...
// behavior that has been removed from momor's runtime in the
...
const test = (name, opts, fn) => {
  if (typeof opts === "function")
    return _test(
      name,
      { skip: "legacy OCR path disabled — see file header" },
      opts,
    );
  return _test(
    name,
    { ...(opts || {}), skip: "legacy OCR path disabled — see file header" },
    fn,
  );
};
...
process.env.momor_TEST_USER_DATA =
  "/Users/alice/Library/Application Support/momor";

const VALID_IMAGE =
  "/Users/alice/Library/Application Support/momor/screenshots/test.png";

async function loadService() {
  const mod = await import(
    pathToFileURL(path.join(screenDir, "ScreenUnderstandingService.js")).href
  );
...
test("ocr_only invokes OCR and never marks vision as requested", async () => {
  const service = await makeService({
    ocrText: "Welcome to the lecture on linear algebra",
  });
  const result = await service.understand(
    baseRequest({
...
test("auto + technical interview + vision provider → vision_direct", async () => {
  const service = await makeService({
    ocrText: "function twoSum(nums, target) { return []; }",
  });
  const result = await service.understand(
    baseRequest({
...
test("auto + general mode + strong OCR → OCR-only path", async () => {
  const service = await makeService({
    ocrText:
      "Quarterly revenue grew by 18 percent compared with prior year results.",
  });
  const result = await service.understand(
    baseRequest({
      modeTemplateType: "general",
...
test("every result includes a provenance value", async () => {
  ...
  const cases = [
    ...
    {
      mode: "vision_only",
      policy: { visionAvailable: true, visionProvider: "gpt-4o" },
    },
  ...
  for (const c of cases) {
    const r = await service.understand(
      baseRequest({
