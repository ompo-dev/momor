...
const test = (name, opts, fn) => {
  if (typeof opts === "function")
    return _test(
      name,
      {
        skip: "legacy OCR-coupled ScreenUnderstandingService API replaced by vision-first pipeline",
      },
      opts,
    );
  return _test(
...
async function loadService() {
  const modPath = pathToFileURL(
    path.join(screenDir, "ScreenUnderstandingService.js"),
  ).href;
...
test("ScreenUnderstandingService rejects invalid image paths without invoking OCR", async () => {
  ...
  const result = await service.understand(
    baseRequest({ imagePaths: ["/etc/passwd"] }),
  );
  ...
  assert.ok(
    result.warnings.some((w) => w.includes("Invalid image path rejected")),
  );
});
