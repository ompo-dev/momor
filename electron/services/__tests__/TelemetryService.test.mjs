...
const { TelemetryService, sanitizeTelemetryProperties } = await import(
  pathToFileURL(telemetryPath).href
);

function makeTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "momor-telemetry-test-"));
}
...
test("API keys and tokens are redacted from properties", () => {
  const sanitized = sanitizeTelemetryProperties({
    ...
    nested: {
      token: "momor_sk_supersecretvalue",
  ...
  assert.doesNotMatch(serialized, /momor_sk_supersecretvalue/);
...
test("transcript, reference, prompt, and screenshot-like fields are removed or replaced", () => {
  ...
  assert.doesNotMatch(
    ...
    /raw transcript|customer private|private-shot/,
  );
});
...
test("provider fallback and error events retain safe metadata but no raw body", () => {
  ...
  service.track({
    ...
    properties: {
      ...
      errorBody:
        '{"error":"contains raw request and sk-abcdefghijklmnopqrstuvwxyz123456"}',
      httpStatus: 429,
    },
  });
  ...
  assert.doesNotMatch(
    ...
    /contains raw request|sk-abcdefghijklmnopqrstuvwxyz123456/,
  );
});
...
test("invalid and unserializable properties do not crash", () => {
  ...
  assert.doesNotThrow(() =>
    service.track({ name: "stt_error", properties: circular }),
  );
  const [record] = readRecords(filePath);
