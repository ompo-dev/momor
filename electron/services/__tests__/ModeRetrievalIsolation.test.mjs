...
describe("Retrieval isolation — only active mode files contribute", () => {
  ...
  test("grounding guard is present in every retrieval that produces snippets", () => {
    ...
    const result = runScenario({
      mode,
      files,
      query: "what is our Q1 ARR run rate for the momor pilot",
      transcript:
        "Founder: what is our Q1 ARR run rate for the pilot accounts? It should be in the metrics sheet.",
    });
    assert.ok(
      result.snippets.length > 0,
      "Test data should produce at least one snippet",
    );
    assert.ok(
  ...
  test("customContext is treated as a retrievable source alongside files", () => {
    const customContext =
      "Founder note: pricing changes effective May 30 — internal-only.";
  ...
  test("snippet source identity is preserved per chunk", () => {
    ...
    for (const s of result.snippets) {
      assert.ok(
        s.sourceId,
        "Each snippet must carry a sourceId for downstream citation",
      );
