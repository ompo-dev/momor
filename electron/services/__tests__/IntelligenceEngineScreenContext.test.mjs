...
async function loadSessionTracker() {
  const sessionPath = path.resolve(
    __dirname,
    "../../../dist-electron/electron/SessionTracker.js",
  );
...
test("runWhatShouldISay passes screenContext and preserves imagePaths", async () => {
  ...
  const calls = [];
  const imagePaths = ["/tmp/momor-test-screen.png"];
  const screenContext = {
...
test("runWhatShouldISay forwards dynamic action promptInstruction to WhatToAnswerLLM", async () => {
  ...
  assert.equal(
    answer,
    "Acknowledge the objection and ask a discovery question.",
  );
  assert.equal(calls.length, 1);
  assert.equal(
    calls[0].promptInstruction,
