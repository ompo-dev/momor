...
async function loadRedactor() {
  const distPath = path.resolve(
    __dirname,
    "../../../dist-electron/electron/utils/redactForLog.js",
  );
  return import(pathToFileURL(distPath).href);
}

test("redactForLog scrubs API keys, bearer tokens, JWTs, and momor/Anthropic key shapes", async () => {
  const { redactForLog } = await loadRedactor();

  const inputs = [
    "auth: Bearer abc123def456ghi789jkl0mn",
    "momor_sk_THIS_SHOULD_BE_HIDDEN",
...
test("redactForLog removes raw transcript / prompt / reference / screenshot fields and redacts sensitive keys", async () => {
  ...
  for (const canary of [
    "CANDIDATE_TRANSCRIPT_CANARY",
    "INTERNAL_PROMPT_CANARY",
    "RESUME_CANARY",
    "CHUNK_CANARY",
    "shot.png",
    "sk-FOO",
    "ANOTHERSECRETTOKEN1234",
  ]) {
    assert.ok(
      !out.includes(canary),
      `canary ${canary} should not leak — got: ${out}`,
    );
  }
...
test("redactForLog handles Error objects, arrays, and cyclic references", async () => {
  ...
  const err = new Error(
    "failure with token sk-XXXXXXXXXXXXXXXXXXXXX inside message",
  );
...
test("main.ts global console wrapper uses redactArgsForLog", () => {
  ...
  assert.match(
    src,
    /console\.log\s*=\s*\(\.\.\.args:\s*any\[\]\)\s*=>\s*\{\s*logToFile\('\[LOG\] '\s*\+\s*redactArgsForLog\(args\)\);/,
  );
  assert.match(
    src,
    /console\.warn\s*=\s*\(\.\.\.args:\s*any\[\]\)\s*=>\s*\{\s*logToFile\('\[WARN\] '\s*\+\s*redactArgsForLog\(args\)\);/,
  );
  assert.match(
    src,
    /console\.error\s*=\s*\(\.\.\.args:\s*any\[\]\)\s*=>\s*\{\s*logToFile\('\[ERROR\] '\s*\+\s*redactArgsForLog\(args\)\);/,
  );
});
