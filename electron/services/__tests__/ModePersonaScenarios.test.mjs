...
import {
  ...
  loadReferenceFiles,
  foreignSentinels,
} from "../../../tests/utils/referenceFileFactory.mjs";
import {
...
describe("Mode: general — five realistic founder/PM scenarios", () => {
  ...
  test("2. Customer onboarding call — audio setup question", () => {
    const result = runWith({
      ...
      customContext: ctx,
      query:
        "customer asks how to configure their audio device for the first meeting",
    });
  ...
  test("3. Internal planning call — Q2 priority question", () => {
    const result = runWith({
      ...
      transcript:
        "PM: lets review the Q2 priority — multi-modal copilot beta — and confirm milestone owners.",
    });
  ...
  test("4. Client update call — project codename pull", () => {
    const result = runWith({
      ...
      transcript:
        "Client: which codename are we using for the Halcyon project brief?",
    });
  ...
  test("5. Founder brainstorming — investor sync date recall", () => {
    const result = runWith({
      ...
      transcript:
        "Founder: remind me when the investor sync with Hyperion Partners is set.",
    });
...
describe("Mode: sales — five realistic sales scenarios", () => {
  ...
  test("1. Pricing objection — Acme discount floor", () => {
    const result = runWith({
      ...
      customContext: ctx,
      query:
        "prospect from Acme says price is too high — what discount floor can we offer",
    });
  ...
  test("4. Buying signal — annual seats prompt", () => {
    const result = runWith({
      ...
      customContext: ctx,
      query:
        "prospect mentioned rolling out annual seats — what is the buying signal next step",
    });
...
describe("Mode: recruiting — five realistic recruiting scenarios", () => {
  ...
  test("1. Backend engineer screen — role fit", () => {
    const result = runWith({
      ...
      customContext: ctx,
      query:
        "what does the backend platform role require around Kafka and PostgreSQL",
    });
  ...
  test("5. Weak signal — candidate channel and ATS lookup", () => {
    const result = runWith({
      ...
      customContext: ctx,
      query:
        "how did candidate ATS-7321 come into our pipeline via referral channel",
      transcript:
        "Hiring manager: was candidate ATS-7321 a referral or job board channel?",
    });
...
describe("Mode: team-meet — five realistic meeting scenarios", () => {
  ...
  test("1. Sprint planning — owner of TM-204", () => {
    const result = runWith({
      ...
      transcript:
        "Lead: lets walk the sprint backlog — TM-204 owned by Sarah due Friday?",
    });
  ...
  test("4. Design review — embeddings storage decision", () => {
    const result = runWith({
      ...
      transcript:
        "PM: confirm the decision — embeddings storage on SQLite with sqlite-vec, right?",
    });
...
describe("Mode: looking-for-work — five realistic candidate scenarios", () => {
  ...
  test("1. Behavioral — tell me about a project you scaled", () => {
    const result = runWith({
      ...
      transcript:
        "Interviewer: walk me through a project you built and scaled — PriceX or momor for example.",
    });
    // Either PriceX or momor scale sentinel is acceptable here.
    const haystack = result.formattedContext;
    assert.ok(
  ...
  test("3. Conflict story — payments vendor escalation", () => {
    const result = runWith({
      ...
      customContext: ctx,
      query:
        "tell me about a conflict story you resolved with a payments vendor",
      transcript:
        "Interviewer: describe a conflict — perhaps the chargeback story with the payments vendor escalation.",
    });
  ...
  test("4. STAR coaching — measurable outcomes anchor", () => {
    const result = runWith({
      ...
      transcript:
        "Coach: STAR stories should always anchor in measurable outcomes — numbers, comparison, scope.",
    });
  ...
  test("5. Post-offer negotiation — target base recall", () => {
    const result = runWith({
      ...
      customContext: ctx,
      query:
        "what target base should I counter with versus my BATNA competing offer",
      transcript:
        "Recruiter: what target base salary are you looking for? Candidate notes: target $185k, BATNA competing offer at $180k.",
    });
    assertSentinelInRetrieval(
      result,
      SENTINELS["looking-for-work"].negotiation,
    );
  });
});
...
describe("Mode: technical-interview — five realistic technical scenarios", () => {
  ...
  test("1. Two-sum-on-sorted-array — problem recall", () => {
    ...
    assertSentinelInRetrieval(
      result,
      SENTINELS["technical-interview"].arrayProblem,
    );
  ...
  test("3. Two-pointer complexity", () => {
    ...
    assertSentinelInRetrieval(
      result,
      SENTINELS["technical-interview"].complexity,
    );
  });
  ...
  test("4. System design — sharded throughput cap", () => {
    ...
    assertSentinelInRetrieval(
      result,
      SENTINELS["technical-interview"].systemDesign,
    );
  });
  ...
  test("5. Debugging — handler error log", () => {
    const result = runWith({
      ...
      customContext: ctx,
      query:
        "what does the error log say about the TypeError at handlers.ts:114",
      transcript:
        "Interviewer: read the error log — TypeError at handlers.ts:114, what is the bug?",
    });
...
describe("Mode: lecture — five realistic lecture/study scenarios", () => {
  ...
  test("2. Definition — Green's function notation", () => {
    const result = runWith({
      ...
      transcript:
        "Lecturer: the green function definition: G satisfies LG=delta. Memorize this.",
    });
  ...
  test("3. Past question — PYQ-2024-Q3 recall", () => {
    const result = runWith({
      ...
      transcript:
        "Student: what did pyq-2024-q3 ask about harmonic boundary problem on a disk?",
    });
...
describe("Mode: negotiation overlay — five realistic negotiation scenarios", () => {
  ...
  test("2. Refund retention — pro-rata policy", () => {
    const result = runWith({
      ...
      customContext: ctx,
      query:
        "customer wants a refund on their annual plan pro-rata policy within 30 days",
      transcript:
        "Customer: I would like a refund on my annual plan; what does the pro-rata policy say within 30 days?",
    });
  ...
  test("5. Vendor negotiation — comparison and SOC2", () => {
    const result = runWith({
      ...
      customContext: ctx,
      query:
        "how does vendor B compare on price and SOC2 compliance against vendor A and vendor C",
      transcript:
        "Procurement: vendor B is 18% lower on price but no SOC2. Vendor A has SOC2. Vendor C bundles premium support.",
    });
