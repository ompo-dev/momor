...
// momor no longer uses it in the default runtime path (vision-first
...
// set momor_RUN_LEGACY_OCR_TESTS=1 to opt in.
// =====================================================================
...
const LEGACY_OCR_ENABLED = process.env.momor_RUN_LEGACY_OCR_TESTS === "1";
const skipReason = LEGACY_OCR_ENABLED
  ? undefined
  : "legacy OCR path disabled — set momor_RUN_LEGACY_OCR_TESTS=1 to run";
const test = (name, opts, fn) => {
  if (typeof opts === "function")
    return _test(name, { skip: skipReason }, opts);
  return _test(name, { ...(opts || {}), skip: skipReason }, fn);
};
const describe = (name, opts, fn) => {
  if (typeof opts === "function")
    return _describe(name, { skip: skipReason }, opts);
...
async function loadScreenModules() {
  const hashPath = pathToFileURL(
    path.join(screenDir, "ImageHashService.js"),
  ).href;
  const contextPath = pathToFileURL(
    path.join(screenDir, "ScreenContextService.js"),
  ).href;
...
test("ImageHashService has required methods", async () => {
  ...
  assert.strictEqual(
    typeof hashService.computeHash,
    "function",
    "should have computeHash",
  );
  assert.strictEqual(
    typeof hashService.quickHash,
    "function",
    "should have quickHash",
  );
});
...
test("ScreenContextService has required methods and interface", async () => {
  ...
  assert.strictEqual(
    typeof service.captureScreen,
    "function",
    "should have captureScreen",
  );
  assert.strictEqual(
    typeof service.captureCropper,
    "function",
    "should have captureCropper",
  );
  assert.strictEqual(
    typeof service.captureScreenFromPath,
    "function",
    "should have captureScreenFromPath",
  );
  assert.strictEqual(
    typeof service.clearCache,
    "function",
    "should have clearCache",
  );
  assert.strictEqual(
    typeof service.getCacheStats,
    "function",
    "should have getCacheStats",
  );
...
test("ScreenContextService captureScreenFromPath handles non-existent file gracefully", async () => {
  ...
  try {
    const result = await service.captureScreenFromPath(
    ...
    assert.ok(
      error.message.includes("Failed to compute"),
...
test("ScreenContextService cache hit updates timestamp without reusing unsafe path input", async () => {
  ...
  const first = await service.captureScreenFromPath(
    "/tmp/original-safe-screen.png",
  );
  const second = await service.captureScreenFromPath(
    "/tmp/../../private/other-screen.png",
  );
