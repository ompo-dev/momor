...
// avoid cross-test interference we set momor_TELEMETRY_TEST_RUN_ID
...
process.env.momor_TELEMETRY_TEST_RUN_ID = RUN_ID;
...
function fallbackEventsSinceOffset(startOffset) {
  return readNewLines(startOffset).filter(
    (e) =>
...
let logOffset = 0;
beforeEach(() => {
  logOffset = fs.existsSync(TELEMETRY_LOG)
    ? fs.statSync(TELEMETRY_LOG).size
    : 0;
...
function makeRetriever({ embeddingReady = false, throwOnEmbed = false } = {}) {
  ...
  const embeddingPipeline = {
    ...
    getEmbedding: async () => {
      if (throwOnEmbed) throw new Error("synthetic embedding failure");
      return [0, 0, 0];
    },
    getEmbeddingForQuery: async () => {
      if (throwOnEmbed) throw new Error("synthetic query embedding failure");
      return [0, 0, 0];
    },
  };
...
describe("FIX-007: Lexical-fallback telemetry", () => {
  ...
  test("throttle: 100 rapid fallback calls emit at most 1 event per (modeId, reason)", async () => {
    ...
    assert.equal(
      events.length,
      1,
      `Throttle must collapse 100 calls into 1 event. Got ${events.length}.`,
    );
  });

  test("static helper: db_unavailable path is throttled the same way", () => {
    const before = fs.existsSync(TELEMETRY_LOG)
      ? fs.statSync(TELEMETRY_LOG).size
      : 0;
    for (let i = 0; i < 50; i++) {
    ...
    assert.equal(
      events.length,
      1,
      `db_unavailable static emitter must throttle. Got ${events.length} events for 50 calls.`,
    );
  });
  ...
  test("throttle keys on (modeId, reason): different modeIds emit independently", async () => {
    ...
    assert.equal(
      events.length,
      2,
      "Throttle must allow one event per distinct modeId",
    );
  ...
  test("telemetry payload carries no raw query / chunk / transcript content (redaction guard)", async () => {
    ...
    assert.ok(
      !serialized.includes("Acme"),
      `Telemetry must not carry customer name. Got:\n${serialized}`,
    );
    assert.ok(
      !serialized.includes("BATNA"),
      "Telemetry must not carry negotiation context",
    );
    assert.ok(
      !serialized.includes("Sarah"),
      "Telemetry must not carry chunk content",
    );
  });
  ...
  test("redaction: extended SENSITIVE_KEY_RE drops query / chunk / userInput / errorMessage values", async () => {
    ...
    const tmod = await import(
      pathToFileURL(
        path.resolve(
          __dirname,
          "../../../dist-electron/electron/services/telemetry/TelemetryService.js",
        ),
      ).href
    );
    const { telemetryService } = tmod;
    const offsetBefore = fs.existsSync(TELEMETRY_LOG)
      ? fs.statSync(TELEMETRY_LOG).size
      : 0;
    ...
    const newEvents = readNewLines(offsetBefore).filter(
      (e) => e?.modeId === "mode_redact",
    );
    assert.equal(newEvents.length, 1);
    const serialized = JSON.stringify(newEvents[0]);
    assert.ok(
      !serialized.includes("should NOT be present"),
