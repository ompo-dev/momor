...
// OcrProviderManager. OCR has been removed from momor's runtime
...
async function loadOcrManager() {
  const mod = await import(
    pathToFileURL(path.join(screenDir, "OcrProviderManager.js")).href
  );
...
async function recognise(imagePath, options = {}) {
  ...
  return manager.recognize(imagePath, {
    timeoutMs: 30_000,
    maxDimension: 1600,
    ...options,
  });
...
test(
  "OcrProviderManager extracts simple sentences from a fixture",
  { skip: SKIP_REAL_OCR },
  async () => {
    ...
    assert.match(
      text,
      /hello/i,
      `expected "hello" in OCR text. Got: ${result.text}`,
    );
    assert.match(
      text,
      /momor/i,
      `expected "momor" in OCR text. Got: ${result.text}`,
    );
...
test(
  "OcrProviderManager recognises a code-problem screenshot",
  { skip: SKIP_REAL_OCR },
  async () => {
    ...
    assert.match(
      text,
      /return/i,
      `expected "return" in OCR text. Got: ${result.text}`,
    );
  },
);

test(
  "OcrProviderManager recognises an error-log screenshot",
  { skip: SKIP_REAL_OCR },
  async () => {
    await ensureFixtures();
...
test(
  "OcrProviderManager recognises a table-style screenshot",
  { skip: SKIP_REAL_OCR },
  async () => {
    await ensureFixtures();
...
test(
  "OcrProviderManager rejects with a clear error when the timeout is impossibly small",
  {
    skip: SKIP_REAL_OCR,
  },
  async () => {
...
test(
  "OcrProviderManager errors cleanly on an invalid image file",
  { skip: SKIP_REAL_OCR },
  async () => {
