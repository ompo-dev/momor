...
export class ReleaseNotesManager {
  ...
  public async fetchReleaseNotes(
    version: string,
    forceRefresh = false,
    ...
    if (
      ...
      console.log(
        "[ReleaseNotesManager] Returning cached release notes for",
        version,
      );
    ...
    console.log(
      `[ReleaseNotesManager] Fetching release notes for ${version}...`,
    );
    ...
    try {
      ...
      if (!response) {
        console.warn(
          "[ReleaseNotesManager] Failed to fetch release notes from API.",
        );
        return null;
      }
      ...
      console.error(
        "[ReleaseNotesManager] Error fetching release notes:",
        error,
      );
      return null;
    }
  ...
  private parseReleaseNotes(
    ...
    console.log(
      `[ReleaseNotesManager] Parsing body for ${version}. Length: ${body.length}`,
    );
    ...
    for (const raw of rawSections) {
      ...
      if (title === "Summary") {
        ...
        console.log(
          `[ReleaseNotesManager] Found Summary: "${summary.substring(0, 50)}..."`,
        );
        ...
        if (items.length > 0) {
          ...
          console.log(
            `[ReleaseNotesManager] Found Section: "${title}" (${items.length} items)`,
          );
        } else {
          console.warn(
            `[ReleaseNotesManager] Section "${title}" found but no valid bullet points extracted.`,
          );
        }
      }
    }
    ...
    return new Promise((resolve) => {
      ...
      request.on("response", (response) => {
        if (response.statusCode !== 200) {
          console.warn(
            `[ReleaseNotesManager] HTTP ${response.statusCode} for ${url}`,
          );
          resolve(null);
          return;
        }
