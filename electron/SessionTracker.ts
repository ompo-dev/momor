...
export class SessionTracker {
  ...
  // Track interim segments to prevent data loss before STT finalizes
  ...
  /** When true, user speech stays in context but must not auto-trigger AI replies. */
  private activeListeningMode = false;
  ...
  setActiveListeningMode(enabled: boolean): void {
    this.activeListeningMode = enabled;
  }

  isActiveListeningMode(): boolean {
  ...
  setCodingQuestion(
    ...
    if (this.detectedCodingQuestion === null) {
      ...
      console.log(`[SessionTracker] Coding question stored`, {
        source,
    ...
    if (source === "screenshot") {
      ...
      this.codingQuestionSource = source;
      this.codingQuestionSetAt = now;
      console.log(`[SessionTracker] Coding question updated via screenshot`, {
        length: trimmed.length,
      });
      return;
    }
    ...
    if (canOverride) {
      ...
      this.codingQuestionSource = source;
      this.codingQuestionSetAt = now;
      console.log(`[SessionTracker] Coding question updated via transcript`, {
        source: this.codingQuestionSource,
        stale: isStale,
        length: trimmed.length,
      });
    } else {
      console.log(
        `[SessionTracker] Transcript question ignored — screenshot question is recent (< ${SessionTracker.SCREENSHOT_STALE_MS / 1000}s)`,
      );
    }
  }
  ...
  getDetectedCodingQuestion(): {
    ...
    source: "screenshot" | "transcript" | null;
  } {
    return {
      question: this.detectedCodingQuestion,
      source: this.codingQuestionSource,
    };
  }
  ...
  addTranscript(
  ...
  ): { role: "interviewer" | "user" | "assistant" } | null {
    ...
    if (!isInternalPrompt) {
      ...
      void this.compactTranscriptIfNeeded().catch((e) =>
        console.warn(
          "[SessionTracker] compactTranscript error (non-fatal):",
          e,
        ),
      );
    }
  ...
  addAssistantMessage(text: string): void {
    console.log(`[SessionTracker] addAssistantMessage called`, {
      length: text.length,
    });

    // momor-style filtering
    if (!text) return;
    ...
    console.log(
      `[SessionTracker] lastAssistantMessage updated, history size: ${this.assistantResponseHistory.length}`,
    );
    this.evictOldEntries();
  }
  ...
  handleTranscript(
    ...
    if (segment.speaker === "user") {
      if (isVerboseLogging() && (Math.random() < 0.05 || segment.final)) {
        console.log(`[SessionTracker] RX User Segment`, {
          final: segment.final,
    ...
    if (segment.speaker === "interviewer") {
      if (isVerboseLogging() && (Math.random() < 0.05 || segment.final)) {
        console.log(`[SessionTracker] RX Interviewer Segment`, {
          final: segment.final,
          length: segment.text.length,
        });
      }
      ...
      if (!segment.final) {
        ...
        this.recentInterviewerBuffer.push({
          text: segment.text,
          timestamp: segment.timestamp,
        });
        const bufferCutoff =
          Date.now() - SessionTracker.INTERVIEWER_BUFFER_WINDOW_MS;
        this.recentInterviewerBuffer = this.recentInterviewerBuffer.filter(
          (e) => e.timestamp >= bufferCutoff,
        );
        ...
        if (this.looksLikeCodingQuestion(segment.text)) {
          ...
          const combinedText = this.recentInterviewerBuffer
            .map((e) => e.text)
            .join(" ");
  ...
  /** Latest spoken line from interviewer or candidate (excludes assistant suggestions). */
  getLastConversationalTurn(): string | null {
    for (let i = this.contextItems.length - 1; i >= 0; i--) {
      const item = this.contextItems[i];
      if (item.role === "interviewer" || item.role === "user") {
        return item.text;
      }
    }
    return null;
  ...
  flushInterimTranscript(): void {
    if (this.lastInterimInterviewer) {
      console.log("[SessionTracker] Force-saving pending interim transcript", {
        length: this.lastInterimInterviewer.text.length,
      });
    ...
    if (this.lastInterimUser) {
      console.log("[SessionTracker] Force-saving pending interim user transcript", {
        length: this.lastInterimUser.text.length,
      });
  ...
  /**
   * Start a fresh meeting session. Clears transcript/context from any prior
   * meeting so STT and LLM never bleed state across sessions.
   */
  beginMeeting(): void {
    ...
    try {
      ...
      if (this.recapLLM) {
        try {
          ...
          console.warn(
            "[SessionTracker] Epoch summarization failed, using fallback marker",
          );
        }
      } else {
        ...
        this.transcriptEpochSummaries.push(marker);
        console.warn(
          "[SessionTracker] recapLLM not available — storing plain epoch marker",
        );
      }

      // Cap epoch summaries to prevent LLM context window overflow
      if (
        this.transcriptEpochSummaries.length >
        SessionTracker.MAX_EPOCH_SUMMARIES
      ) {
        this.transcriptEpochSummaries = this.transcriptEpochSummaries.slice(
          -SessionTracker.MAX_EPOCH_SUMMARIES,
        );
      }
