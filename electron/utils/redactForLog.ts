...
const VALUE_PATTERNS: Array<{ regex: RegExp; replacement: string }> = [
  ...
  {
    regex:
      /x-(momor|trial|api)-(key|token)\s*[:=]\s*[A-Za-z0-9._~+\/=:-]{8,}/gi,
    replacement: "$&[REDACTED]".replace(
      /(=|:)\s*[A-Za-z0-9._~+\/=:-]{8,}/,
      "$1 [REDACTED]",
    ),
  },
  { regex: /momor_sk_[A-Za-z0-9._-]+/gi, replacement: REDACTED },
...
function sanitize(value: unknown, seen: WeakSet<object>): unknown {
  ...
  if (typeof value === "string")
    return scrubString(value).slice(0, MAX_PREVIEW_LEN);
