...
const MAC_USER_DATA = "/Users/alice/Library/Application Support/momor";
...
describe("validateImagePath — macOS userData ordering (obs 2631)", () => {
  ...
  test("blocks /Users/ path outside userData", () => {
    const r = validateImagePath(
      "/Users/alice/Desktop/secret.png",
      MAC_USER_DATA,
    );
    assert.equal(
      r.isValid,
      false,
      "arbitrary user home paths must remain blocked",
    );
  });
  ...
  test("blocks path traversal even when prefix matches userData", () => {
    ...
    assert.equal(
      r.isValid,
      false,
      "traversal escape must be blocked even under userData prefix",
    );
  });
