...
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ------------------------- | --------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | ------------------------------------------- | -------------------------------------------------------------------------------------- |
| **momor API**                     | yes                                                                                                                   | multipart/form-data; image first resized with Sharp to ≤768 px on long edge, base64 encoded                                  | no — only forwards        | yes (momor backend treats as multimodal)                  | server-side fallback                                                                               | medium (cloud)                              | `generateWithmomor` `LLMHelper.ts:1698`; `streamWithmomor`                             |
...
```
momor → codex → openai → gemini_flash → claude → gemini_pro → groq
```
...
```
momor → groq → codex → gemini_flash → gemini_pro → openai → claude
```
...
- Scope policy: `assertOutboundScopes('momor', text, imagePaths)` runs at
...
- Gemini Pro is in the multimodal list but listed _after_ Flash, so latency
...
- **No data-scope assertion** runs for this path
  (compare to `LLMHelper.ts:1699` for momor). A screenshot can leave the
...
the default in the multimodal stream list (after momor/Codex/OpenAI),
...
layer still applies because the momor path is short-circuited for
