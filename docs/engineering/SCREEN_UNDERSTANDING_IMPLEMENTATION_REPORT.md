...
momor now has a production-ready screen understanding pipeline with:
...
- Screen context status chip (lines 3101-3104 in momorInterface.tsx)
...
1. **`ScreenUnderstandingService` is still not called from `IntelligenceEngine`,
   ...
   behaviour at the _service_ level and in tests, but does not yet route the
...
3. **No UI consumer** for `provenance` / `ocrRan` / `visionRequested`. The
   service exposes everything; no `momorInterface` chip or answer pill
