# momor → Cluely Parity Roadmap

**Companion to:** `momor_CLUELY_PARITY_FIX_LOG.md`
**Purpose:** track everything _not_ completed in the current pass with concrete
...
- **1.1 Trial token leak through IPC**
  ...
  - DoD: renderer cannot reconstruct any sk\_/jwt-shaped string; secret stays in main only.
  - Effort: 0.5 day.
...
- **3.2 UI cards (DynamicActionBar / DynamicActionCard / ActionEvidencePopover)**
  - Files: create `src/components/dynamic-actions/{DynamicActionBar,DynamicActionCard,ActionEvidencePopover}.tsx`; mount above answer area in `momorInterface.tsx`.
...
- **4.2 UI indicator**
  - Files: `src/components/momorInterface.tsx`.
...
- **Submodules `momor-api` and `premium`:** modified on entry, untouched here.
