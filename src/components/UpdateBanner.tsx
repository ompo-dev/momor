...
const UpdateBanner: React.FC = () => {
  ...
  const [status, setStatus] = useState<
    "idle" | "downloading" | "ready" | "error" | "instructions"
  >("idle");
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const [instructionsArch, setInstructionsArch] = useState<
    "arm64" | "x64" | null
  >(null);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.metaKey && !e.shiftKey && e.key.toLowerCase() === "i") {
        e.preventDefault();
        console.log(
          "[UpdateBanner] Cmd+I pressed: Triggering Test Release Fetch...",
        );
      ...
      if (e.metaKey && !e.shiftKey && e.key.toLowerCase() === "j") {
        e.preventDefault();
        console.log(
          "[UpdateBanner] Cmd+J pressed: Triggering Instruction UI mock...",
        );
        setUpdateInfo({ version: "2.0.8" });
        setParsedNotes({
          summary: "Test Update",
          fullBody: "Testing",
          sections: [{ title: "Notes", items: ["UI Test"] }],
        });
        setStatus("idle");
        setIsVisible(true);
      }
    ...
    if (window.electronAPI.platform === "darwin") {
      try {
        ...
        const version = updateInfo?.version
          ...
          : "2.0.8";
        const url = `https://github.com/momor-AI-assistant/momor-cluely-ai-assistant/releases/download/v${version}/Momor-${version}-${dmgSuffix}.dmg`;
