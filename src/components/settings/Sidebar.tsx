...
interface SidebarProps {
  activeTab: "general" | "momor-api" | "ai-providers" | "about";
  setActiveTab: (
    tab: "general" | "momor-api" | "ai-providers" | "about",
  ) => void;
  onClose: () => void;
}
    ...
    <div className="w-64 bg-bg-sidebar flex flex-col border-r border-border-subtle h-full">
      <div className="p-6">
        ...
        <nav className="space-y-1">
          ...
          <button
            onClick={() => setActiveTab("momor-api")}
            className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-3 ${activeTab === "momor-api" ? "bg-bg-item-active text-text-primary" : "text-text-secondary hover:text-text-primary hover:bg-bg-item-active/50"}`}
          >
            <MomorLogoMark size={16} className="text-blue-500" /> Momor API
          </button>
          <button
