...
const MockAppInterface = () => {
  ...
  useEffect(() => {
    const id = setInterval(
      () => setActiveBtn((i) => (i + 1) % MOCK_BUTTONS.length),
      1600,
    );
          ...
          <div className="relative w-[90%] mx-auto pt-2">
            ...
            >
              <span className="text-text-secondary inline-flex items-center text-[13px] italic leading-7 opacity-60">
                ...and I'd also consider a distributed cache layer for
                horizontal scaling
          ...
          <div className="p-4 space-y-3 pb-2 flex-1 overflow-y-auto max-h-[220px]">
            ...
            <div className="flex justify-end">
              <div className="max-w-[72.25%] px-[13.6px] py-[10.2px] text-[14px] leading-relaxed whitespace-pre-wrap bg-blue-500/10 border border-blue-500/20 text-blue-500 rounded-[20px] rounded-tr-[4px] shadow-sm font-medium">
                <span className="font-semibold text-emerald-500 block mb-1 text-[12px]">
                  🎯 Answer
                </span>
                A good approach would be to use a hash map to cache intermediate
                results and reduce time complexity to O(N).
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-nowrap justify-center items-center gap-1.5 px-4 pb-3 pt-3 overflow-x-hidden">
            {MOCK_BUTTONS.map((btn, idx) => {
              ...
              return (
                <button
                  key={idx}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-medium border transition-all duration-300 whitespace-nowrap shrink-0 ${isActive ? colorMap[btn.color] : "bg-bg-item-surface text-text-primary border-border-subtle"}`}
                >
              ...
              <div className="w-full border border-border-subtle rounded-xl pl-3 pr-10 py-2.5 text-[13px] leading-relaxed bg-bg-input shadow-inner flex items-center h-[46px]">
                <div className="absolute left-3 top-1/2 -translate-y-1/2 flex items-center gap-1.5 pointer-events-none text-[13px] text-text-secondary opacity-60">
                  ...
                  <div className="flex items-center gap-1 opacity-80 sm:ml-0.5">
                    <kbd className="px-1.5 py-0.5 rounded border text-[10px] font-sans min-w-[20px] text-center bg-bg-item-surface border-border-subtle text-text-primary shadow-sm">
                      ⌘
                    </kbd>
    ...
    <div className="w-full aspect-[3/2] bg-bg-secondary rounded-[20px] border border-border-subtle overflow-hidden flex flex-col relative shadow-lg select-none pointer-events-none">
      ...
      <div className="flex items-center justify-between px-6 pt-4 pb-3 shrink-0">
        <div className="p-1 rounded-xl inline-flex items-center gap-0.5 bg-bg-input border border-border-subtle">
          {["summary", "transcript", "usage"].map((t) => (
            <button
              key={t}
              className={`relative px-3 py-1 text-[12px] font-medium rounded-lg z-10 transition-colors ${tab === t ? "text-text-primary bg-bg-elevated shadow-sm border border-border-subtle" : "text-text-tertiary"}`}
            >
            ...
            >
              ...
              <div className="mb-5 pb-5 border-b border-border-subtle">
                <p className="text-sm text-text-secondary leading-relaxed">
                  Discussed microservice architecture for the new payment
                  gateway. Analyzed Redis vs Memcached for caching with a focus
                  on data persistence to prevent race conditions during
                  checkout.
                </p>
              </div>
              ...
              <section className="mb-6">
                ...
                <ul className="space-y-3">
                  {[
                    "Draft Redis implementation constraints doc.",
                    "Schedule follow-up on Memcached benchmarks.",
                  ].map((item, i) => (
              ...
              <section>
                ...
                <ul className="space-y-3">
                  {[
                    "Redis chosen for sorted set support enabling O(log N) rate limiting.",
                    "Horizontal scaling via distributed cache layer discussed.",
                  ].map((item, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <div className="mt-2 w-1.5 h-1.5 rounded-full bg-text-secondary shrink-0" />
                      <p className="text-sm text-text-secondary leading-relaxed">
              ...
              {[
                {
                  speaker: "Them",
                  time: "10:32",
                  text: "Why did you use Redis over Memcached for the cart session?",
                },
                {
                  speaker: "Me",
                  time: "10:33",
                  text: "Because we needed sorted sets for rate limiting and automatic expiry without custom cron jobs.",
                },
              ].map((entry, i) => (
                <div key={i}>
              ...
              <div className="flex items-start gap-3">
                <div className="mt-0.5 w-5 h-5 rounded-full bg-bg-input flex items-center justify-center border border-border-subtle shrink-0">
                  <img
                    src={momorIcon}
                    alt="AI"
                    className="w-3 h-3 opacity-50 object-contain force-black-icon"
                  />
                </div>
                <div>
                  <div className="text-[10px] text-text-tertiary mb-1 font-medium">
                    10:35 AM
                  </div>
                  <p className="text-xs text-text-secondary leading-relaxed">
                    You mentioned Redis Sorted Sets for the sliding rate window
                    — efficient because it auto-expires stale records while
                    keeping operations strictly O(log N).
                  </p>
                </div>
              </div>
            </motion.div>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 flex justify-center">
        <div className="w-full max-w-[440px] flex items-center relative">
          <div className="w-full pl-4 pr-11 py-2.5 bg-bg-item-surface shadow-sm border border-border-subtle rounded-full text-xs text-text-tertiary/70">
    ...
    <div className="w-full bg-bg-secondary rounded-[20px] border border-border-subtle overflow-hidden flex flex-col select-none pointer-events-none shadow-lg max-h-[280px]">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border-subtle shrink-0">
        <div className="flex items-center gap-2 text-text-tertiary">
          <img
            src={momorIcon}
            className="w-3.5 h-3.5 force-black-icon opacity-50"
            alt="logo"
          />
      ...
      <div className="p-5 space-y-5">
        ...
        <div className="flex flex-col items-start">
          <p className="text-sm text-text-primary leading-relaxed max-w-[85%]">
            Based on the transcript near 10:45 AM, they explicitly mentioned
            integrating{" "}
            <code className="bg-bg-tertiary px-1.5 py-0.5 rounded text-[12px] font-mono text-text-primary border border-border-subtle">
              Stripe Payment Intents
            </code>{" "}
            to handle the recurring tier logic securely.
          </p>
        ...
        <div className="relative flex items-center border-b border-border-muted">
          ...
          <div className="w-full bg-transparent pl-9 pr-4 py-2.5 text-[13px] text-text-primary outline-none flex items-center h-[38px]">
            ...
            <motion.span
              ...
              className="w-[1.5px] h-3.5 bg-blue-500 ml-[2px] inline-block"
            />
          </div>
        </div>
            ...
            <div className="px-3 py-1 mt-1">
              ...
              <div className="w-full flex items-center gap-3 px-2 py-1.5 rounded-lg text-left hover:bg-bg-item-hover transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-text-tertiary">Jan 12</div>
                </div>
              </div>
              ...
              <div className="w-full flex items-center gap-3 px-2 py-1.5 rounded-lg text-left hover:bg-bg-item-hover transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-text-tertiary">Jan 08</div>
                </div>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="flex justify-center flex-col items-center gap-4 py-8 bg-bg-card rounded-xl border border-border-subtle relative overflow-hidden h-[240px]">
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2">
            <img
              src={momorIcon}
              alt="Momor"
              className="w-6 h-6 object-contain rounded drop-shadow-sm opacity-90"
            />
            <span className="text-text-primary text-sm font-medium">Momor</span>
          </div>

          <motion.div
            initial={false}
            animate={{
              backgroundColor: toggled ? "#3b82f6" : "var(--bg-toggle-switch)",
            }}
      ...
      <div className="text-xs text-text-secondary text-center max-w-[280px]">
        Momor requires Accessibility and Screen Recording permissions to analyze
        screen context.
      </div>
    </div>
  );
...
const MockPillControlsAnim = () => {
  ...
  return (
    <div className="mt-4 space-y-2.5">
      <div className="text-[10px] font-semibold text-text-tertiary uppercase tracking-wider px-1 mb-3">
        Pill Controls
      </div>
      ...
      <div className="flex items-center gap-3 p-3 bg-bg-elevated border border-border-subtle rounded-xl">
        <div className="w-8 h-8 rounded-full bg-bg-item-active flex items-center justify-center border border-border-muted shrink-0 shadow-sm">
          <img
            src={momorIcon}
            alt="Logo"
            className="w-[18px] h-[18px] object-contain force-black-icon opacity-90"
          />
        </div>
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <span className="text-[11px] text-text-secondary leading-snug">
            — clicking this brings you right back
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 p-3 bg-bg-elevated border border-border-subtle rounded-xl">
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <span className="text-[11px] text-text-secondary leading-snug">
            Toggles entire window — keeps you{" "}
            <strong className="text-text-primary">purely stealth</strong>
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center gap-4 z-10">
        ...
        <div className="text-center">
          ...
          <div className="text-xs text-text-secondary mt-1">
            Routing via Groq LPU (Response &lt; 0.5s)
          </div>
        </div>
      </div>
  ...
  const options = [
    {
      ...
      badge: "Default",
      recommended: true,
      desc: "On-device transcription — no API key required",
      color: "emerald",
    ...
    },
    {
      ...
      badge: "Saved",
      recommended: false,
      desc: "Real-time streaming with code-switching support",
      color: "purple",
      icon: <Mic size={14} />,
    },
    {
    ...
    },
    {
      ...
      badge: "",
      recommended: false,
      desc: "Fast LPU whisper transcription",
      color: "orange",
      icon: <Mic size={14} />,
    },
    {
      ...
      badge: "",
      recommended: false,
      desc: "Enterprise tier transcription",
      color: "teal",
      icon: <Mic size={14} />,
    },
    {
      ...
      badge: "",
      recommended: false,
      desc: "Medical-grade transcription",
      color: "cyan",
      icon: <Mic size={14} />,
    },
    {
  ...
  return (
    <div className="flex justify-center flex-col items-center py-6 bg-bg-card rounded-xl border border-border-subtle relative overflow-hidden h-[300px]">
      <div className="w-[340px] flex flex-col gap-2 relative z-10 font-sans">
        ...
        <div className="relative">
          <button
            className={`w-full group bg-bg-input border border-border-subtle shadow-sm rounded-xl p-2.5 pr-3.5 flex items-center justify-between transition-all duration-200 outline-none ${isOpen ? "ring-2 ring-accent-primary/20 border-accent-primary/50" : "hover:shadow-md"}`}
          >
            <div className="flex items-center gap-3 overflow-hidden">
              <div
                className={`w-9 h-9 rounded-[10px] flex items-center justify-center shrink-0 transition-all duration-300 transform ${getIconStyle(selected.color, false)}`}
              >
                {selected.icon}
              </div>
              <div className="min-w-0 flex-1 text-left">
                <div className="flex items-center gap-2">
                  ...
                  {selected.badge && (
                    <span
                      className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wide ml-2 ${getBadgeStyle("green")}`}
                    >
                      {selected.badge}
                    </span>
                  )}
          ...
          <AnimatePresence>
            {isOpen && (
              <motion.div
                ...
                className={
                  "absolute top-full left-0 w-full mt-2 backdrop-blur-xl rounded-xl shadow-2xl overflow-hidden z-20 bg-bg-elevated border border-border-subtle"
                }
              >
                <div
                  className="max-h-[170px] overflow-hidden relative"
                  style={{
                    WebkitMaskImage:
                  ...
                  >
                    {options.map((option) => {
                      ...
                      return (
                        <div
                          key={option.id}
                          className={`w-full rounded-[10px] p-2 flex items-center gap-3 transition-all duration-200 group relative cursor-pointer ${isSelected ? "bg-bg-item-active shadow-inner" : "hover:bg-bg-item-hover"}`}
                        >
                          <div
                          ...
                          <div className="flex-1 min-w-0 text-left">
                            <div className="flex items-center justify-between mb-0.5">
                              <div className="flex items-center gap-2">
                                <span
                                  className={
                                    "text-[13px] font-medium transition-colors text-text-primary"
                                  }
                                >
                                  {option.label}
                                </span>
                                {option.badge && (
                                  <span
                                    className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wide ${getBadgeStyle("green")}`}
                                  >
                                    {option.badge}
                                  </span>
                                )}
                            ...
                            <span
                              className={
                                "text-[11px] block truncate transition-colors text-text-secondary"
                              }
                            >
                              {option.desc}
                            </span>
        ...
        <div className="flex items-center justify-between mt-2">
          <div className="flex items-center gap-3">
            <div className="text-xs bg-bg-input px-3 py-1.5 rounded-md flex items-center gap-2 border border-border-subtle shadow-sm">
              ...
              <span
                className={stage > 2 ? "text-green-500" : "text-text-primary"}
              >
                {stage === 2
                  ? "Testing..."
                  : stage > 2
          ...
          <div
            className={`p-2 rounded-lg ${stage === 4 ? "bg-red-500/20 text-red-500" : "text-text-tertiary"} border border-transparent`}
          >
            <Trash2 size={16} />
          </div>
        </div>
  ...
  return (
    <div className="w-full flex justify-center py-4 bg-bg-elevated rounded-xl border border-border-subtle mb-3 mt-2 shadow-sm">
      <div className="flex items-center justify-between w-full max-w-[360px]">
        <span className="text-[14.5px] text-text-primary font-medium tracking-tight">
          Speech to Text
        </span>
        <div className="flex items-center bg-bg-main p-[3px] rounded-lg border border-border-subtle shadow-inner">
          <div className="px-3.5 py-1.5 text-[13px] font-medium text-text-secondary">
            No Access
          </div>
          <div className="px-3.5 py-1.5 text-[13px] font-medium text-black bg-white rounded-md shadow-sm relative z-10 before:absolute before:inset-0 before:rounded-md before:border-[1.5px] before:border-black before:opacity-90 before:-m-[1px]">
            Access
          </div>
        </div>
      </div>
...
const AccordionSection: React.FC<AccordionSectionProps> = ({
  ...
  return (
    <div
      className={`border rounded-xl mb-4 overflow-hidden transition-all duration-200 bg-bg-card border-border-subtle shadow-sm`}
    >
      <button
      ...
      >
        <div className="flex items-center gap-3">
          <div
            className={`w-8 h-8 rounded-lg flex items-center justify-center bg-bg-item-surface border border-border-subtle group-hover:border-border-muted transition-colors text-text-secondary`}
          >
            {icon}
          </div>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div
              className={`p-5 border-t border-border-subtle text-sm leading-relaxed text-text-secondary`}
            >
              {children}
            </div>
          </motion.div>
...
const SetupGuide = () => {
  ...
  const steps = [
    {
      title: t("help.steps.permissions.title"),
      desc: t("help.steps.permissions.desc"),
    },
    {
      title: t("help.steps.audio.title"),
      desc: t("help.steps.audio.desc"),
    },
    {
      title: t("help.steps.aiModel.title"),
      desc: t("help.steps.aiModel.desc"),
    },
    {
      title: t("help.steps.personalize.title"),
      desc: t("help.steps.personalize.desc"),
    },
    {
      title: t("help.steps.allSet.title"),
      desc: null as string | null,
    },
  ];

  const hotkeys = [
    { label: t("help.hotkeys.toggle"), kbd: "⌘H" },
    { label: t("help.hotkeys.screenshot"), kbd: "⌘⇧H" },
    { label: t("help.hotkeys.chat"), kbd: "⌘K" },
  ];
          ...
          return (
            <div key={i} className="flex gap-4">
              ...
              >
                ...
                {!isLast && (
                  <div
                    className="w-px bg-border-subtle flex-1"
                    style={{ minHeight: 32, marginTop: 5, marginBottom: 5 }}
                  />
                )}
              </div>
              ...
              {/* Content */}
              <div
                className={`flex-1 min-w-0 ${isLast ? "pb-0" : "pb-6"}`}
                style={{ paddingTop: 3 }}
              >
                    ...
                    {hotkeys.map((h, hi) => (
                      <React.Fragment key={h.kbd}>
                        <div className="flex items-center gap-1.5">
                          ...
                          <kbd className="font-mono text-[11px] font-semibold text-text-primary bg-bg-item-surface border border-border-subtle rounded-md px-1.5 py-0.5 leading-none">
                            {h.kbd}
                          </kbd>
                        </div>
                        {hi < hotkeys.length - 1 && (
                          <span className="text-border-subtle text-[12px] select-none">
                            ·
                          </span>
                        )}
                      </React.Fragment>
...
export const HelpSettings: React.FC<{ onNavigate?: (tab: string) => void }> = ({
  ...
  // Kbd class applying theme variables momor
  ...
  return (
    <div className="w-full h-full flex flex-col animated fadeIn pb-10">
      ...
      <div className="flex-1 space-y-2">
          ...
          >
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 flex-1">
              ...
              <div className="flex-1">
                <h4 className="text-[14px] font-bold text-text-primary mb-0.5">
                  {t("help.configureIntegrations.title")}
                </h4>
                <p className="text-[13px] text-text-secondary">
                  {t("help.configureIntegrations.desc")}
                </p>
              </div>
            </div>
        ...
        <AccordionSection
          title={t("help.sections.permissions")}
          icon={<Monitor className="w-4 h-4" />}
        >
          <div className="space-y-4">
            <p>{t("help.permissions.intro")}</p>
            <MockPermissionsAnim />
            <div className="space-y-3 mt-4">
              <h4 className="font-bold text-base text-text-primary border-b border-border-subtle pb-2">
                {t("help.permissions.hardwareTitle")}
              </h4>

              <div
                className={`p-4 rounded-xl border bg-bg-item-surface border-border-subtle space-y-2`}
              >
                <h5
                  className={`font-semibold text-[13px] text-text-primary flex items-center gap-2`}
                >
                  <Mic size={14} className="text-blue-500" />{" "}
                  {t("help.permissions.micTitle")}
                </h5>
                <p className="text-[11px] opacity-90 leading-relaxed text-text-secondary">
                  {t("help.permissions.micDesc")}
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                >
                  ...
                  >
                    ...
                    {t("help.permissions.sckTitle")}
                  </h5>
                  <p className="text-[11px] opacity-90 leading-relaxed text-text-secondary">
                    {t("help.permissions.sckDesc")}
                  </p>
                </div>
                <div
                  className={`p-4 rounded-xl border bg-bg-item-surface border-border-subtle space-y-2`}
                >
                  ...
                  >
                    ...
                    {t("help.permissions.coreAudioTitle")}
                  </h5>
                  <p className="text-[11px] opacity-90 leading-relaxed text-text-secondary">
                    {t("help.permissions.coreAudioDesc")}
                  </p>
                </div>
              </div>
              ...
              >
                ...
                >
                  ...
                  {t("help.permissions.languageTitle")}
                </h5>
                <p className="text-[11px] opacity-90 leading-relaxed text-text-secondary">
                  {t("help.permissions.languageDesc")}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3 mt-6">
              <div
                className={`p-4 rounded-xl border bg-bg-item-surface border-border-subtle`}
              >
                <h4
                  className={`font-semibold text-sm mb-2 text-text-primary flex items-center gap-2`}
                >
                  <Monitor className="w-4 h-4 text-accent-primary" />{" "}
                  {t("help.permissions.screenTitle")}
                </h4>
                <p className="text-xs opacity-90 mb-2">
                  {t("help.permissions.screenDesc")}
                </p>
                <p className="text-[11px] text-text-tertiary">
                  {t("help.permissions.screenPath")}
                </p>
              </div>
              ...
              >
                ...
                >
                  ...
                  {t("help.permissions.a11yTitle")}
                </h4>
                <p className="text-xs opacity-90 mb-2">
                  {t("help.permissions.a11yDesc")}
                </p>
                <p className="text-[11px] text-text-tertiary">
                  {t("help.permissions.a11yPath")}
                </p>
              </div>
            </div>
          </div>
        ...
        <AccordionSection
          title={t("help.sections.audioStt")}
          icon={<Mic className="w-4 h-4" />}
        >
          <div className="space-y-6">
            <p>{t("help.audioStt.intro")}</p>

            <MockProviderSelectionAnim />

            <div className="space-y-4 pt-2">
              <h4 className="font-bold text-lg text-text-primary border-b border-border-subtle pb-2">
                {t("help.audioStt.apiTitle")}
              </h4>
              <p className="text-xs text-text-secondary">
                {t("help.audioStt.apiDesc")}
              </p>
            ...
            <div className="space-y-3 pt-4">
              ...
              <div className="p-4 rounded-xl border bg-bg-item-surface border-border-subtle space-y-2">
                <h5 className="font-semibold text-sm text-text-primary flex justify-between items-center">
                  ...
                  <button
                    ...
                    className="text-accent-primary hover:underline text-[10px] flex items-center gap-1"
                  >
                    <ExternalLink size={10} /> Link
                  </button>
                </h5>
                <p className="text-xs opacity-90 leading-relaxed text-text-secondary">
                  Uses a Service Account JSON instead of an API Key. You must
                  build a GCP Project, activate the Cloud Speech API, and create
                  a Service Account under IAM. Download the JSON Key, and
                  drag-and-drop it into the box in the Audio Settings.
                </p>
              </div>
              ...
              <div className="p-4 rounded-xl border bg-bg-item-surface border-border-subtle space-y-2">
                ...
                <p className="text-xs opacity-90 leading-relaxed text-text-secondary">
                  When creating a Custom API key for the "Conversational AI"
                  streaming integration, you must explicitly enable{" "}
                  <strong>convai.conversations.create</strong> and{" "}
                  <strong>assistants.list</strong>. Crucially, you must also
                  allow <strong>Speech to Text: Access</strong> as seen below in
                  the ElevenLabs UI:
                </p>
              ...
              <div className="p-4 rounded-xl border bg-bg-item-surface border-border-subtle space-y-2">
                ...
                <p className="text-xs opacity-90 leading-relaxed text-text-secondary">
                  Provides ultra-fast Whisper responses via LPUs. Starts with{" "}
                  <span className={kbdClass}>gsk_</span>. No special permissions
                  required.
                </p>
              </div>
              ...
              <div className="p-4 rounded-xl border bg-bg-item-surface border-border-subtle space-y-2">
                ...
                <p className="text-xs opacity-90 leading-relaxed text-text-secondary">
                  Uses standard OpenAI keys (
                  <span className={kbdClass}>sk-</span>). Remember: this Audio
                  key is isolated from your AI Generation key under "AI
                  Providers".
                </p>
              </div>
              ...
              <div className="p-4 rounded-xl border bg-bg-item-surface border-border-subtle space-y-2">
                ...
                <p className="text-xs opacity-90 leading-relaxed text-text-secondary">
                  Extremely accurate streaming transcription (Nova-3 with
                  multilingual code-switching support). Key is generated
                  on-demand in the Deepgram Console.
                </p>
              </div>
              ...
              <div className="p-4 rounded-xl border bg-bg-item-surface border-border-subtle space-y-2">
                ...
                <p className="text-xs opacity-90 leading-relaxed text-text-secondary">
                  Requires establishing Azure Speech Services.{" "}
                  <strong>Important</strong>: You must also specify an Azure
                  Region alongside your key (e.g. <em>eastus</em>,{" "}
                  <em>westeurope</em>) or requests will bounce.
                </p>
              </div>
              ...
              <div className="p-4 rounded-xl border bg-bg-item-surface border-border-subtle space-y-2">
                ...
                <p className="text-xs opacity-90 leading-relaxed text-text-secondary">
                  Create a Watson Speech to Text resource in IBM Cloud and
                  generate a set of API credentials.
                </p>
              </div>
              ...
              <div className="p-4 rounded-xl border bg-bg-item-surface border-border-subtle space-y-2">
                ...
                <p className="text-xs opacity-90 leading-relaxed text-text-secondary">
                  Provides high-end medical/law tier transcriptions using their
                  standard dashboard API Key.
                </p>
              </div>
            </div>
          </div>
        ...
        <AccordionSection
          title={t("help.sections.aiProviders")}
          icon={<Key className="w-4 h-4" />}
        >
          <div className="space-y-4">
            <p className="text-sm">
              Momor uses Large Language Models (LLMs) to reason about your
              screen and audio context. You can configure cloud providers, local
              models, or fully custom endpoints.
            </p>

            <div className="space-y-3 pt-2">
              <h4 className="font-bold text-lg text-text-primary border-b border-border-subtle pb-2">
                1. Standard Cloud Providers
              </h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="p-3 rounded-xl border bg-bg-item-surface border-border-subtle hover:border-border-muted transition-colors">
                  <h5 className="font-semibold text-sm text-text-primary flex justify-between items-center mb-1">
                    <span className="flex items-center gap-2">
                      <img
                        src="https://groq.com/favicon.svg"
                        alt="Groq"
                  ...
                  <p className="text-[11px] opacity-80 mb-2">
                    Ultra-fast inference using LPU hardware. Default model:{" "}
                ...
                <div className="p-3 rounded-xl border bg-bg-item-surface border-border-subtle hover:border-border-muted transition-colors">
                  <h5 className="font-semibold text-sm text-text-primary flex justify-between items-center mb-1">
                    <span className="flex items-center gap-2">
                      <SiOpenai
                        className={`w-3.5 h-3.5 ${isLight ? "text-black" : "text-white"}`}
                      />{" "}
                      OpenAI
                    </span>
                    <button
                  ...
                  <p className="text-[11px] opacity-80 mb-2">
                    Industry standard pipeline. Default models:{" "}
                    <strong>gpt-5.4-mini</strong> & <strong>gpt-5.4</strong>.
                  </p>
                ...
                <div className="p-3 rounded-xl border bg-bg-item-surface border-border-subtle hover:border-border-muted transition-colors">
                  ...
                  <p className="text-[11px] opacity-80 mb-2">
                    Superior coding baseline parameters. Default:{" "}
                    <strong>claude-4.6-sonnet</strong>.
                  </p>
                ...
                <div className="p-3 rounded-xl border bg-bg-item-surface border-border-subtle hover:border-border-muted transition-colors">
                  <h5 className="font-semibold text-sm text-text-primary flex justify-between items-center mb-1">
                    <span className="flex items-center gap-2">
                      <SiGoogle className="w-3.5 h-3.5 text-blue-500" /> Google
                      Gemini
                    </span>
                    <button
                      onClick={() => {
                  ...
                  <p className="text-[11px] opacity-80 mb-2">
                    Immense contextual window. Default model:{" "}
              ...
              <div className="mt-2 bg-bg-item-surface p-4 rounded-xl border border-border-subtle shadow-sm flex gap-3">
                ...
                <p className="text-[11px] text-text-secondary leading-relaxed mt-0.5">
                  ...
                  momor utilizes a 14-day background sync clock (
                  <span className="font-mono bg-bg-elevated border border-border-muted px-1.5 py-0.5 rounded text-[10px] text-text-primary shadow-[inset_0_-1px_0_rgba(0,0,0,0.1)]">
                    v2/api/models
                  </span>
                  ) to silently poll upstream APIs. If Anthropic or OpenAI drops
                  a new flagship architecture (e.g. GPT-5), your app dynamically
                  absorbs it into the UI dropdown automatically.
                </p>
              </div>

              <div className="p-4 mt-2 rounded-xl border border-border-subtle bg-bg-item-surface">
                <h5 className="font-semibold text-[13px] text-text-primary mb-1">
                  Configuring the Active Model Engine
                </h5>
                <p className="text-[11px] text-text-secondary leading-relaxed">
                  Inside the Launcher UI (above the start button), you can
                  hot-swap your <strong>Active Model</strong>. This dictation is
                  extremely important—it determines the active core reasoning
                  engine. If set to <strong>claude-3-5-sonnet</strong>, the
                  intelligence agent uses Anthropic infrastructure exclusively
                  for screen analysis. Switch to <strong>llama3:8b</strong>{" "}
                  beneath it, and the architecture instantly reverts to
                  generating responses via your offline GPU pipeline.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3 pt-4">
              ...
              <div className="p-4 rounded-xl border bg-bg-item-surface border-border-subtle space-y-3">
                <p className="text-xs opacity-90 leading-relaxed text-text-secondary">
                  You can run Momor completely offline with 100% data privacy
                  using Ollama. Momor automatically scans{" "}
                  <span className={kbdClass}>http://localhost:11434</span> for
                  active models.
                </p>
                <ol className="list-decimal pl-4 text-xs space-y-2 opacity-90 text-text-secondary">
                  <li>
                    Download Ollama locally via{" "}
                    <button
                      onClick={() => {
                        (window as any).electronAPI?.openExternal(
                      ...
                      className="text-accent-primary hover:underline inline-flex items-center gap-1 font-medium"
                    >
                      ollama.com <ExternalLink size={10} />
                    </button>
                  </li>
                  <li>
                    Open Terminal and run our recommended 8B parameter
                    instruction model:
                    <div className="mt-1 bg-bg-input p-2 rounded border border-border-subtle font-mono text-[11px]">
                      ollama run llama3:8b
                    </div>
                  </li>
                  <li>
                    Altermomor, for faster generation without GPU, use
                  ...
                  <li>
                    Return to Momor's AI Providers overlay, and you will see
                    your Local models ready for usage.
                  </li>
                </ol>
              </div>
            </div>
            ...
            <div className="space-y-3 pt-4">
              ...
              <div className="p-4 rounded-xl border bg-bg-item-surface border-border-subtle space-y-3">
                <p className="text-xs opacity-90 leading-relaxed text-text-secondary">
                  Use Custom Providers to hook up any standard external LLM
                  router (like OpenRouter, LMStudio, or proprietary company
                  endpoints). Create a new provider using a cURL command
                  template.
                </p>
                ...
                <div className="flex items-start gap-2 mt-2">
                  <div className="w-5 h-5 rounded bg-orange-500/20 text-orange-500 flex items-center justify-center shrink-0 mt-0.5">
                    <Zap size={10} />
                  </div>
                  <div className="text-xs text-text-secondary leading-relaxed">
                    <strong>Crucial: The Response Path.</strong> You must inform
                    Momor how to parse the JSON text back. Deeply nested outputs
                    must define the exact path array. For OpenAI/OpenRouter
                    compliant endpoints, this is strictly:{" "}
                    <span className={kbdClass}>choices[0].message.content</span>
                    .
                  </div>
                </div>
              </div>
        ...
        <AccordionSection
          title={t("help.sections.interfaceOps")}
          icon={<Monitor className="w-4 h-4" />}
        >
          <div className="space-y-6">
            <p className="text-[13px]">{t("help.interfaceOps.intro")}</p>
            ...
            <div className="mt-4 mb-3 flex items-center gap-3">
              ...
              <span className="text-[10px] font-semibold text-text-tertiary uppercase tracking-widest">
                {t("help.interfaceOps.quickActions")}
              </span>
              <div className="flex-1 h-px bg-border-subtle" />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {(
                [
                  {
                    ...
                    bc: "",
                    kbd: ["⌘", "1"],
                    desc: "Reads the active transcript and screen, then streams a precise response to read aloud.",
                  },
                  {
                    Icon: Lightbulb,
                    color: "violet",
                  ...
                  },
                  {
                    ...
                    bc: "",
                    kbd: ["⌘", "4"],
                    desc: "Suggests the next logical question to keep conversation flowing gracefully.",
                  },
                  {
                    Icon: Zap,
                    color: "emerald",
                    ...
                    bc: "",
                    kbd: ["⌘", "5"],
                    desc: "Records your mic + screen context and fires an immediate AI query.",
                  },
                  {
                    Icon: MessageSquare,
                    color: "indigo",
                    ...
                    bc: "",
                    kbd: ["⌘", "2"],
                    desc: "Generates sharp probing questions from latent audio when a topic is unclear.",
                  },
                  {
                    Icon: RefreshCw,
                    color: "amber",
                    title: "Recap",
                    badge: "Interview OFF",
                    bc: "bg-red-500/10 text-red-400 border-red-500/30",
                    kbd: ["⌘", "3"],
                    desc: "Condenses the last 5 minutes into bullet points when you lose the thread.",
                  },
                  {
                    Icon: Sparkles,
                    color: "sky",
                    ...
                    bc: "",
                    kbd: ["⌘", "6"],
                    desc: "Reads your screen and nudges you toward the correct code implementation.",
                  },
                  {
                    Icon: Monitor,
                    color: "rose",
                    ...
                    bc: "",
                    kbd: ["⌘", "⇧", "H"],
                    desc: "Forces a full-screen capture and immediately processes it through the LLM.",
                  },
                  {
                    Icon: EyeOff,
                    color: "slate",
                    title: "Stealth Execute",
                    badge: null,
                    bc: "",
                    kbd: ["⌘", "↵"],
                    desc: "Processes context in the background without ever revealing the interface.",
                  },
                ] as Array<{
              ...
              ).map(({ Icon, color, title, badge, bc, kbd, desc }) => {
                const isWindows =
                ...
                const tone = {
                  blue: {
                    ...
                    glow: "group-hover:shadow-[0_0_0_1px_rgba(59,130,246,0.2),0_4px_12px_rgba(59,130,246,0.07)]",
                  },
                  violet: {
                    bg: "bg-violet-500/10",
                    text: "text-violet-400",
                    border: "border-violet-500/20",
                    glow: "group-hover:shadow-[0_0_0_1px_rgba(139,92,246,0.2),0_4px_12px_rgba(139,92,246,0.07)]",
                  },
                  teal: {
                    bg: "bg-teal-500/10",
                    text: "text-teal-400",
                    border: "border-teal-500/20",
                    glow: "group-hover:shadow-[0_0_0_1px_rgba(20,184,166,0.2),0_4px_12px_rgba(20,184,166,0.07)]",
                  },
                  emerald: {
                    bg: "bg-emerald-500/10",
                    text: "text-emerald-400",
                    border: "border-emerald-500/20",
                    glow: "group-hover:shadow-[0_0_0_1px_rgba(16,185,129,0.2),0_4px_12px_rgba(16,185,129,0.07)]",
                  },
                  indigo: {
                    bg: "bg-indigo-500/10",
                    text: "text-indigo-400",
                    border: "border-indigo-500/20",
                    glow: "group-hover:shadow-[0_0_0_1px_rgba(99,102,241,0.2),0_4px_12px_rgba(99,102,241,0.07)]",
                  },
                  amber: {
                    bg: "bg-amber-500/10",
                    text: "text-amber-400",
                    border: "border-amber-500/20",
                    glow: "group-hover:shadow-[0_0_0_1px_rgba(245,158,11,0.2),0_4px_12px_rgba(245,158,11,0.07)]",
                  },
                  sky: {
                    bg: "bg-sky-500/10",
                    text: "text-sky-400",
                    border: "border-sky-500/20",
                    glow: "group-hover:shadow-[0_0_0_1px_rgba(14,165,233,0.2),0_4px_12px_rgba(14,165,233,0.07)]",
                  },
                  rose: {
                    bg: "bg-rose-500/10",
                    text: "text-rose-400",
                    border: "border-rose-500/20",
                    glow: "group-hover:shadow-[0_0_0_1px_rgba(244,63,94,0.2),0_4px_12px_rgba(244,63,94,0.07)]",
                  },
                  slate: {
                    bg: "bg-slate-500/10",
                    text: "text-slate-400",
                    border: "border-slate-500/20",
                    glow: "group-hover:shadow-[0_0_0_1px_rgba(100,116,139,0.2),0_4px_12px_rgba(100,116,139,0.07)]",
                  },
                }[color];
                ...
                return (
                  <div
                    key={title}
                    className={`group flex flex-col gap-1.5 p-3 rounded-xl border border-border-subtle bg-bg-item-surface hover:bg-bg-elevated transition-all duration-200 cursor-default ${tone.glow}`}
                  >
                    {/* Line 1 — Icon + Name */}
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-5 h-5 rounded-md ${tone.bg} border ${tone.border} flex items-center justify-center shrink-0`}
                      >
                        <Icon
                          className={`w-3 h-3 ${tone.text}`}
                          strokeWidth={2.5}
                        />
                      </div>
                      <span className="text-[12px] font-bold text-text-primary tracking-tight leading-none truncate">
                        {title}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center">
                      {badge ? (
                        <span
                          className={`text-[9px] font-black uppercase tracking-wider px-1.5 py-[2px] border rounded leading-none ${bc}`}
                        >
                          {badge}
                        </span>
                      ) : (
                        <span className="text-[9px] font-bold text-text-tertiary/50 uppercase tracking-wider leading-none">
                          {t("help.interfaceOps.alwaysActive")}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-1">
                      {resolvedKbd.map((key, i) => (
                        <span
                          key={i}
                          className="px-1.5 py-0.5 rounded text-[10px] font-mono inline-block bg-bg-elevated text-text-secondary"
                        >
                          {key}
                        </span>
                      ))}
        ...
        <AccordionSection
          title={t("help.sections.meetingIntel")}
          icon={<FileText className="w-4 h-4" />}
        >
          <div className="space-y-6">
            <p className="text-[13px]">
              {t("help.meetingIntel.intro")}
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="p-4 bg-bg-item-surface border border-border-subtle rounded-xl shadow-sm hover:border-border-muted transition-colors group">
                <h4 className="text-[14px] font-bold text-text-primary flex items-center gap-2 mb-2">
                  ...
                  {t("help.meetingIntel.summaryTitle")}
                </h4>
                <p className="text-[12px] text-text-secondary leading-relaxed">
                  {t("help.meetingIntel.summaryDesc")}
                </p>
              </div>
              ...
              <div className="p-4 bg-bg-item-surface border border-border-subtle rounded-xl shadow-sm hover:border-border-muted transition-colors group">
                <h4 className="text-[14px] font-bold text-text-primary flex items-center gap-2 mb-2">
                  ...
                  {t("help.meetingIntel.transcriptsTitle")}
                </h4>
                <p className="text-[12px] text-text-secondary leading-relaxed">
                  {t("help.meetingIntel.transcriptsDesc")}
                </p>
              </div>
              ...
              <div className="p-4 bg-bg-item-surface border border-border-subtle rounded-xl shadow-sm hover:border-border-muted transition-colors group">
                <h4 className="text-[14px] font-bold text-text-primary flex items-center gap-2 mb-2">
                  ...
                  {t("help.meetingIntel.storageTitle")}
                </h4>
                <p className="text-[12px] text-text-secondary leading-relaxed">
                  {t("help.meetingIntel.storageDesc")}
                </p>
              </div>
            </div>
            ...
            <div className="border-t border-border-subtle pt-6">
              <h4 className="font-bold text-sm text-text-primary flex items-center gap-2 mb-4">
                ...
                {t("help.meetingIntel.semanticTitle")}
              </h4>
              <p className="text-[13px] mb-6">
                {t("help.meetingIntel.semanticDesc")}
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-6">
                <div className="p-4 bg-bg-item-surface border border-border-subtle rounded-xl shadow-sm hover:border-border-muted transition-colors group">
                  <h4 className="text-[14px] font-bold text-text-primary flex items-center gap-2 mb-2">
                    ...
                    {t("help.meetingIntel.contextualTitle")}
                  </h4>
                  <p className="text-[12px] text-text-secondary leading-relaxed">
                    {t("help.meetingIntel.contextualDesc")}
                  </p>
                </div>

                <div className="p-4 bg-bg-item-surface border border-border-subtle rounded-xl shadow-sm hover:border-border-muted transition-colors group">
                  <h4 className="text-[14px] font-bold text-text-primary flex items-center gap-2 mb-2">
                    <Brain className="w-4 h-4 text-teal-500 group-hover:scale-110 transition-transform" />{" "}
                    {t("help.meetingIntel.memoryTitle")}
                  </h4>
                  <p className="text-[12px] text-text-secondary leading-relaxed">
                    {t("help.meetingIntel.memoryDesc")}
                  </p>
                </div>
              </div>
            </div>
        ...
        <AccordionSection
          title={t("help.sections.globalSearch")}
          icon={<Search className="w-4 h-4" />}
        >
          <div className="space-y-6">
            <p className="text-[13px]">{t("help.globalSearch.intro")}</p>

            <div className="p-4 rounded-xl border bg-bg-item-surface border-border-subtle space-y-3">
              <h4 className="font-bold text-sm text-text-primary">
                {t("help.launcherHome.title")}
              </h4>
              <p className="text-[12px] text-text-secondary leading-relaxed">
                {t("help.launcherHome.intro")}
              </p>
              <ul className="text-[12px] text-text-secondary space-y-2 list-disc pl-4">
                <li>{t("help.launcherHome.times")}</li>
                <li>{t("help.launcherHome.refresh")}</li>
                <li>{t("help.launcherHome.searchHeader")}</li>
              </ul>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4 mb-4">
              <div className="p-4 bg-bg-item-surface border border-border-subtle rounded-xl shadow-sm hover:border-border-muted transition-colors group">
                <h4 className="text-[14px] font-bold text-text-primary flex items-center gap-2 mb-2">
                  <Briefcase className="w-4 h-4 text-sky-500 group-hover:scale-110 transition-transform" />{" "}
                  {t("help.globalSearch.meetingTraversalTitle")}
                </h4>
                <p className="text-[12px] text-text-secondary leading-relaxed">
                  {t("help.globalSearch.meetingTraversalDesc")}
                </p>
              </div>
              ...
              <div className="p-4 bg-bg-item-surface border border-border-subtle rounded-xl shadow-sm hover:border-border-muted transition-colors group">
                <h4 className="text-[14px] font-bold text-text-primary flex items-center gap-2 mb-2">
                  ...
                  {t("help.globalSearch.fallbackTitle")}
                </h4>
                <p className="text-[12px] text-text-secondary leading-relaxed">
                  {t("help.globalSearch.fallbackDesc")}
                </p>
              </div>
            </div>

            <div className="border-t border-border-subtle pt-6">
              <h4 className="font-bold text-sm text-text-primary border-b border-border-subtle pb-1">
                {t("help.globalSearch.shortcutsTitle")}
              </h4>
              <p className="text-[11px] text-text-secondary mt-1 mb-3">
                {t("help.globalSearch.shortcutsDesc")}
              </p>
              ...
              <div className="grid gap-3">
                <div className="flex items-center justify-between p-4 rounded-xl border bg-bg-item-surface border-border-subtle group">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <div className="font-semibold text-sm text-text-primary">
                        {t("help.globalSearch.showHideTitle")}
                      </div>
                      <div className="text-xs text-text-secondary mt-1">
                        {t("help.globalSearch.showHideDesc")}
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-1 shrink-0">
                    {(shortcuts.toggleVisibility || ["⌘", "B"]).map(
                      (key: string, i: number) => (
                        <span key={i} className={kbdClass}>
                          {key}
                        </span>
                      ),
                    )}
                ...
                <div className="flex items-center justify-between p-4 rounded-xl border bg-bg-item-surface border-border-subtle group">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <div className="font-semibold text-sm text-text-primary">
                        {t("help.globalSearch.screenshotTitle")}
                      </div>
                      <div className="text-xs text-text-secondary mt-1">
                        {t("help.globalSearch.screenshotDesc")}
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-1 shrink-0">
                    {(shortcuts.takeScreenshot || ["⌘", "H"]).map(
                      (key: string, i: number) => (
                        <span key={i} className={kbdClass}>
                          {key}
                        </span>
                ...
                <div className="flex items-center justify-between p-4 rounded-xl border bg-bg-item-surface border-border-subtle group">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <div className="font-semibold text-sm text-text-primary">
                        Process Captured Context (Execute)
                      </div>
                      <div className="text-xs text-text-secondary mt-1">
                        Triggers Momor to analyze the captured screenshots and
                        text from the rolling buffer.
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-1 shrink-0">
                    {(shortcuts.processScreenshots || ["⌘", "Enter"]).map(
                      (key: string, i: number) => (
                        <span key={i} className={kbdClass}>
                          {key}
                        </span>
                ...
                <div className="flex items-center justify-between p-4 rounded-xl border bg-bg-item-surface border-border-subtle group">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <div className="font-semibold text-sm text-text-primary">
                        Capture + Execute Instantly
                      </div>
                      <div className="text-xs text-text-secondary mt-1">
                        Captures a screenshot AND processes it in one fluid
                        action.
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-1 shrink-0">
                    {(shortcuts.captureAndProcess || ["⌘", "⇧", "Enter"]).map(
                      (key: string, i: number) => (
                        <span key={i} className={kbdClass}>
                          {key}
                        </span>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <h4 className="font-bold text-sm text-text-primary flex items-center gap-2">
                <Zap className="w-4 h-4 text-orange-500" /> Fast Mode (Hardware
                LPU)
              </h4>
              <MockFastModeAnim />
              <div className="p-4 rounded-xl border bg-orange-500/10 border-orange-500/20">
                ...
                <p className="text-xs text-orange-400/80 m-0">
                  Fast Mode activates highly efficient models (like Llama 3 8B)
                  layered over Groq hardware LPUs instead of GPUs. This brings
                  latency down from 2-3 seconds to less than 500ms. To enable
                  Fast Mode, navigate to Settings &gt; General Settings popup
                  overlay, and click the Lightning Icon.
                </p>
              </div>
            </div>
          </div>
        ...
        <AccordionSection
          title={t("help.sections.phoneMirror")}
          ...
          <div className="space-y-4">
            <div className="p-3 bg-sky-500/10 border border-sky-500/20 rounded-xl mb-2">
              <h4 className="text-[13px] font-semibold text-sky-400 flex items-center gap-2 mb-1">
                <Smartphone size={14} /> Stream Momor to Your Phone
              </h4>
              <p className="text-[11px] text-text-secondary leading-relaxed mb-0">
                Phone Mirror spins up a small local web server so you can watch
                Momor's live transcript and AI answers from your phone or tablet
                — handy when your screen is being shared and you don't want the
                overlay visible.
              </p>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-3">
              <div className="p-4 rounded-xl border bg-bg-item-surface border-border-subtle">
                ...
                <ul className="text-[11px] text-text-secondary space-y-1 list-disc pl-4">
                  <li>
                    Open <strong>Settings → Phone Mirror</strong>
                  </li>
                  <li>
                    Toggle on, then scan the generated QR code from your phone
                  </li>
                  <li>
                    Loopback by default — flip <strong>Expose on LAN</strong> to
                    reach it from another device on the same Wi-Fi
                  </li>
                </ul>
              </div>
              <div className="p-4 rounded-xl border bg-bg-item-surface border-border-subtle">
                <h4 className="font-semibold text-sm mb-2 text-text-primary flex items-center gap-2">
                  <Lock className="w-4 h-4 text-amber-500" /> Token-Gated
                  Security
                </h4>
                <p className="text-[11px] text-text-secondary">
                  Every session is protected by a single-use bearer token baked
                  into the QR. If a device leaves your trust circle, hit{" "}
                  <strong>Rotate Token</strong> to invalidate every existing
                  connection in one tap.
                </p>
              </div>
            </div>
            ...
            <div className="p-3 border border-orange-500/20 bg-orange-500/5 rounded-lg">
              <p className="text-[10px] text-orange-400 m-0">
                <strong>⚠️ LAN exposure:</strong> Only enable{" "}
                <em>Expose on LAN</em> on networks you trust. The token stops
                casual snoops, but anyone who captures the QR or copies the URL
                can read your live transcript until you rotate.
              </p>
            </div>
          </div>
        </AccordionSection>
        ...
        >
          <div className="space-y-4">
            <div className="p-3 bg-indigo-500/10 border border-indigo-500/20 rounded-xl mb-4">
              ...
              <p className="text-[11px] text-text-secondary leading-relaxed mb-0">
                Momor is heavily geared towards power users seeking minimalistic
                operation. The process completely disguises itself and remains
                undetectable/invisible to standard screen-recording applications
                and desktop sharing utilities.
              </p>
            </div>
            ...
            <div className="grid gap-3">
              <div className="p-4 rounded-xl border bg-bg-item-surface border-border-subtle group">
                <h4 className="font-semibold text-sm mb-2 text-text-primary flex items-center gap-2">
                  <EyeOff className="w-4 h-4 text-text-secondary" /> Dynamic UI
                  Opacity
                </h4>
                <p className="text-xs text-text-secondary">
                  Decrease your visual footprint to near-zero. Navigate to the
                  General Settings overlay and dynamically drag the{" "}
                  <strong>Opacity Slider</strong> down to make the interface
                  completely translucent against your underlying native
                  applications.
                </p>
              </div>
              ...
              <div className="p-4 rounded-xl border bg-bg-item-surface border-border-subtle group">
                <h4 className="font-semibold text-sm mb-2 text-text-primary flex items-center gap-2">
                  <Monitor className="w-4 h-4 text-text-secondary" /> Mouse
                  Pass-Through Mode
                </h4>
                <p className="text-[11px] text-text-secondary mb-2">
                  Do you want the AI prompt completely fused into your screen
                  without obstructing your clicks? Activate{" "}
                  <strong>Mouse Pass-through</strong> inside the UI toggle menu.
                </p>
                <div className="p-2 border border-orange-500/20 bg-orange-500/5 rounded-lg">
                  <p className="text-[10px] text-orange-400 m-0">
                    <strong>⚠️ Warning:</strong> This renders the Momor overlay
                    completely unclickable. You MUST memorize the Global Hotkeys
                    (e.g. <strong>Cmd+Shift+Arrows</strong> to move,{" "}
                    <strong>Cmd+B</strong> to hide, <strong>Cmd+1-7</strong> for
                    actions) to control the application once this is active.
                  </p>
                </div>
              </div>
            </div>
