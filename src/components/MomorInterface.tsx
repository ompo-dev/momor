...
import { OverlayModelSelect } from "./overlay/OverlayModelSelect";
import { OverlaySttSelect } from "./overlay/OverlaySttSelect";
import { OverlayAiProfileSelect } from "./overlay/OverlayAiProfileSelect";
...
import { CallVadEndpoint, ANSWER_CALL_PAUSE_DEFAULTS, getAnswerCallEndpointPauseMs } from "../lib/callVadEndpoint";
import {
...
interface momorInterfaceProps {
  onEndMeeting?: () => void;
  overlayOpacity?: number;
  interfaceTheme?: MeetingInterfaceTheme;
}

const AUTO_SUGGEST_COOLDOWN_MS = 3000;

function hasInterviewerQuestionSignal(text: string): boolean {
...
const isUserMicSpeaker = (speaker?: string | null): boolean => {
...
const MomorInterface: React.FC<momorInterfaceProps> = ({
  onEndMeeting,
  ...
  const [answerCallSessionActive, setAnswerCallSessionActive] = useState(false);
  ...
  const finishAnswerNowTurnRef =
    useRef<((resumeAfter?: boolean) => Promise<void>) | null>(null);
  ...
  const [rollingCommitted, setRollingCommitted] = useState(""); // Interviewer finalized segments
  const [rollingLive, setRollingLive] = useState(""); // Interviewer live partial
  const rollingLiveRef = useRef("");
  const [isInterviewerSpeaking, setIsInterviewerSpeaking] = useState(false); // Track if actively speaking
  const [userRollingCommitted, setUserRollingCommitted] = useState(""); // User mic finalized segments
  const [userRollingLive, setUserRollingLive] = useState(""); // User mic live partial
  ...
  const [hideChatHidesWidget, setHideChatHidesWidget] = useState(() => {
    const stored = localStorage.getItem("momor_hideChatHidesWidget");
  ...
  const [llmPrivacyLabel, setLlmPrivacyLabel] = useState<string>(
    "Checking privacy route",
  );
  ...
  const [currentSttProfileId, setCurrentSttProfileId] = useState<string | null>(
    null,
  );
  const [currentSttLabel, setCurrentSttLabel] = useState("STT");
    ...
    return window.electronAPI.onSttStatusChanged((data) => {
      if (data.channel === "user") {
        setSttUserStatus((prev) =>
          prev === "failed" && data.state === "reconnecting" ? prev : data.state,
        );
      ...
      window.electronAPI.onNativeAudioTranscript((transcript) => {
        ...
        if (transcript.final) {
          ...
          if (
            ...
            Date.now() - lastAutoSuggestAtRef.current >= AUTO_SUGGEST_COOLDOWN_MS
          ) {
        ...
        return (
          ...
          >
            ...
            >
              ...
              <span>{t('overlay.followUpQuestions')}</span>
            </div>
            <div
              className={`text-[13px] leading-relaxed markdown-content ${isLightTheme ? "text-slate-800" : "text-slate-200"}`}
            >
            ...
            >
              ...
              <div className="relative no-drag flex flex-wrap items-center justify-center gap-1.5 px-4 pt-3 pb-1">
                <div
                  ...
                  title={
                    activeModeLabel
                      ? t('overlay.activeModeLabel', { name: activeModeLabel })
                      : t('overlay.noActiveModeSelected')
                  }
                >
                  ...
                  <span>
                    {activeModeLabel
                      ? t('overlay.activeModeLabel', { name: activeModeLabel })
                      : t('overlay.generalMode')}
                  </span>
                </div>
                <div
                  className={`${statusPillBaseClass} ${getStatusToneClass(sttSummary.tone)}`}
                  title={sttSummaryDetail}
                >
                  <Mic className="h-3 w-3 opacity-70" />
                  <span>{t(sttSummary.labelKey)}</span>
                </div>
                {(() => {
                  ...
                  const visionFailed =
                    ...
                    !!latestVisionFailureReason;
                  const visionSucceeded =
                    (latestUsedImageInput ||
                      screenContextStatus === "available") &&
                    !visionFailed;
                  const failureLabelMap: Record<string, string> = {
                    no_vision_provider: t('overlay.noVisionProvider'),
                    all_vision_failed: t('overlay.visionFailed'),
                    privacy_blocked: t('overlay.privateBlocked'),
                    scope_blocked: t('overlay.screenshotsDisabled'),
                    provider_timeout: t('overlay.visionTimedOut'),
                  };
                  const failureLabel = latestVisionFailureReason
                    ? failureLabelMap[latestVisionFailureReason] ||
                      t('overlay.visionFailed')
                    : t('overlay.visionFailed');
                  const failureTitleMap: Record<string, string> = {
                    no_vision_provider:
                      "No vision-capable provider is configured. Add a Momor, OpenAI, Claude, Gemini, or Groq key — or configure a local Ollama vision model.",
                    all_vision_failed:
                      ...
                      title={
                        attachedContext.length > 0
                          ...
                          : visionFailed
                            ...
                            : visionSucceeded
                              ? providerTitle
                              : t('overlay.noScreenContext')
                      }
                    >
                      <Monitor className="h-3 w-3 opacity-70" />
                      <span>
                        {attachedContext.length > 0
                          ? t('overlay.screenAttached', { count: attachedContext.length })
                          : visionFailed
                            ? failureLabel
                            : visionSucceeded
                              ? providerLabel
                ...
                <div className="flex items-center justify-between mx-4 mt-3 mb-1 px-3.5 py-2.5 bg-yellow-500/10 border border-yellow-500/20 rounded-[12px] shadow-sm relative no-drag group/warning">
                  <div className="flex flex-col gap-1 pr-3">
                    <div className="flex items-center gap-2 text-[12.5px] text-yellow-600 dark:text-yellow-400/90 font-medium leading-tight">
                      ...
                      <span>{t('overlay.screenRecordingDenied')}</span>
                    </div>
              ...
              <div
                className={cn(
                  "max-w-full px-3 pb-2",
                  rollingTranscriptPreview && showTranscript ? "pt-1" : "pt-2",
                )}
              >
                <div className="flex items-center gap-1 overflow-x-auto pb-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                  ...
                  <Button
                    type="button"
                    variant={
                      isManualRecording && !isCallMode ? "destructive" : "secondary"
                    }
                    size="sm"
                    ...
                    className={cn(
                      "h-7 min-w-[72px] shrink-0 rounded-full px-2.5 text-[11px] font-medium",
                  ...
                  >
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-[11px] font-medium overlay-text-primary">
                        {t('overlay.screenshotsAttached', { count: attachedContext.length })}
                      </span>
                      <button
                ...
                {attachedContext.length === 0 && (
                  <div className="relative group" data-stealth-engage="true">
                    <input
                      ...
                      placeholder={t("overlay.typeQuestion")}
                      className={`h-9 w-full rounded-xl border py-1.5 pl-3 pr-10 text-[12px] leading-snug transition-all duration-200 ease-sculpted focus:outline-none focus:ring-1 ${inputClass} ${stealthTapActive ? "border-emerald-400/40 shadow-[0_0_12px_rgba(52,211,153,0.15)] ring-2 ring-emerald-400/30" : ""}`}
                      style={appearance.inputStyle}
                    />
                ...
                <div className="mt-2 flex items-center justify-between px-0.5">
                  <div className="flex items-center gap-1.5">
                    ...
                    <OverlaySttSelect
                      ...
                      onSelect={handleSttProfileSelect}
                      className={controlSurfaceClass}
                      controlStyle={appearance.controlStyle}
                    />
                  ...
                  <Button
                    ...
                    className={cn(
                      "h-7 w-7 rounded-full interaction-base interaction-press",
                      !inputValue.trim() &&
                        attachedContext.length === 0 &&
