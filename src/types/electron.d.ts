...
export interface ElectronAPI {
  ...
  setWhisperXConfig: (config: {
    url: string;
    enabled: boolean;
  }) => Promise<{ success: boolean }>;
  getWhisperXConfig: () => Promise<{ url: string; enabled: boolean }>;
  ...
  getOpenClaudeAuthStatus: (cliPath?: string) => Promise<{
    success: boolean;
    status?: {
      loggedIn: boolean;
      authMethod: string;
  ...
  openClaudeAuthLogout: (cliPath?: string) => Promise<{
    success: boolean;
    message?: string;
    error?: string;
  }>;
  getOpenClaudeModels: () => Promise<{
    success: boolean;
    models: Array<{
      value: string;
      label: string;
  ...
  getStoredCredentials: () => Promise<{
    ...
    claudeBackupKeys?: string[];
    deepseekBackupKeys?: string[];
    sttGroqKey?: string;
    sttOpenaiKey?: string;
  ...
  finalizeMicSTT: () => Promise<void | { success: boolean; error?: string }>;
  startAnswerNowMic: () => Promise<void | { success: boolean; error?: string }>;
  stopAnswerNowMic: () => Promise<void | { success: boolean; error?: string }>;
  ...
  onIntelligenceDynamicAction: (
    callback: (data: { action: DynamicActionPayload }) => void,
  ) => () => void;
  acceptDynamicAction: (actionId: string) => Promise<{
    success: boolean;
    action?: DynamicActionPayload;
    error?: string;
  }>;
  dismissDynamicAction: (
    actionId: string,
  ) => Promise<{ success: boolean; error?: string }>;
  listDynamicActions: () => Promise<{
    success: boolean;
    actions: DynamicActionPayload[];
    error?: string;
  }>;
  ...
  phoneMirrorEnable: (
    exposeOnLan: boolean,
  ) => Promise<PhoneMirrorInfo | { error: string }>;
  ...
  onPhoneMirrorStatus: (
    callback: (info: PhoneMirrorInfo) => void,
  ) => () => void;
}
