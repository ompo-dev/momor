...
const buildAllMacTargets = process.env.momor_BUILD_ALL_MAC_ARCHES === "1";

function verifyArtifacts(expectedArtifacts) {
  const missing = expectedArtifacts.filter(
    (file) => !fs.existsSync(path.join(nativeModulePath, file)),
  );

  if (missing.length > 0) {
    throw new Error(
      `Missing native artifacts after build: ${missing.join(", ")}`,
    );
  }
...
function getClangLibPath() {
  ...
  try {
    const resourceDir = execSync("clang -print-resource-dir", {
      encoding: "utf8",
    }).trim();
...
function fixMacOSDylibPaths(nodeFilePath) {
  try {
    ...
    const otoolOutput = execSync(`otool -L "${nodeFilePath}"`, {
      encoding: "utf8",
    });
    ...
    for (const line of lines) {
      ...
      execSync(
        `install_name_tool -change "${dylibPath}" "${relativePath}" "${nodeFilePath}"`,
      );
    }
    ...
    console.warn(
      `Warning: Could not fix dylib paths for ${path.basename(nodeFilePath)}: ${err.message}`,
    );
  }
}
...
if (os.platform() === "darwin") {
  ...
  for (const target of macTargets) {
    try {
      ...
      console.warn(
        `Warning: Could not configure rust target ${target}. Continuing anyway.`,
      );
    }
    ...
    runCommand(
      `npx napi build --platform --target ${target} --release`,
      extraEnv,
    );
  }
