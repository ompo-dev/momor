 ...
 * shows "CoreServices Helper" instead of "momor Helper".
 *
  ...
  if (fs.existsSync(unpackedNativeDir)) {
    ...
    for (const file of files) {
      if (file.endsWith(".node")) {
        ...
        console.log(
          `[Ad-Hoc Signing] Re-signing ${file} with entitlements (post --deep)...`,
        );
        try {
          execSync(
            `codesign --force --entitlements "${entitlementsPath}" --sign - "${nodePath}"`,
            { stdio: "inherit" },
          );
        } catch (error) {
