You are now the autonomous QA architect, senior test engineer, product engineer, and code reviewer for momor.
...
Mission:
momor currently fixes reported bugs, but the testing is not deep enough. I want you to build a real automated mode/profile-intelligence test system that simulates real users, real resumes, real job descriptions, custom prompts, negotiation contexts, reference files, and long meeting/interview scenarios.

This should test whether each momor mode actually works like a real product, not just whether functions compile.
...
15. momor API integration if configured
...
- If real momor API credentials are configured locally, use them safely.
...
Sales mode sentinel:
"momor Pro annual enterprise discount floor is 17 percent for Acme test accounts."
...
"The candidate built PriceX, a price comparison website, and scaled momor to 10k users."
...
- If the resume says PriceX/momor, answers can use those facts only if present in fixture.
...
1. Behavioral interview
   - Resume: PriceX, momor, open-source
   - JD: SWE role
...
Phase 9: Real momor API testing

If real momor API is configured locally:
...
If real momor API is not configured:
...
15. Real momor API results if run.
16. UI/E2E results.
...
This should become a real momor QA system.
