...
const default_zero_native_path ="C:\\Users\\maico\\AppData\\Local\\npm-cache\\_npx\\5bf20bdb39c17d42\\node_modules\\zero-native";
const app_exe_name = "zero-native-poc";
...
pub fn build(b: *std.Build) void {
    ...
    const dev = b.addSystemCommand(&.{ "npx", "zero-native", "dev", "--manifest", "app.zon", "--binary" });
    ...
    if (platform == .macos) {
        switch (web_engine) {
            ...
            .chromium => {
                ...
                if (cef_auto_install) {
                    const cef_auto = b.addSystemCommand(&.{ "npx", "zero-native", "cef", "install", "--dir", cef_dir });
...
fn addCefCheck(b: *std.Build, target: std.Build.ResolvedTarget, cef_dir: []const u8) *std.Build.Step.Run {
    const script = switch (target.result.os.tag) {
        ...
        .windows => b.fmt(
        \\$ok = (Test-Path "{s}/include/cef_app.h") -and (Test-Path "{s}/Release/libcef.dll") -and (Test-Path "{s}/libcef_dll_wrapper/libcef_dll_wrapper.lib")
        \\if (-not $ok) {{
        \\  Write-Error "missing CEF dependency for -Dweb-engine=chromium"
        \\  Write-Error "Fix with: npx zero-native cef install --dir {s}"
        \\  exit 1
        \\}}
        , .{ cef_dir, cef_dir, cef_dir, cef_dir }),
    ...
    if (target.result.os.tag == .windows) {
        return b.addSystemCommand(&.{ "powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", script });
    }
